...
The plan intentionally asks for a longer DAG timeout and five retry attempts on
inventory reservation. The `reserve_inventory` tool author declares a five-second
timeout and three attempts with `@workflow_tool`; those decorator declarations
are authoritative. Preserving the resource unchanged makes that precedence
visible in the final workflow status.
