...
param mcpServerConfigName string = 'Office-365-Outlook-calendar-email'
...
resource office365McpServerConfig 'Microsoft.Web/connectorGateways/mcpserverconfigs@2026-05-01-preview' = {
  ...
  properties: {
    state: 'Enabled'
    description: 'Office 365 Outlook connector for reading calendar events and sending email.'
    connectors: [
      {
        name: 'office365'
        connectionName: office365Connection.name
        displayName: 'Office 365 Outlook'
        description: 'Read calendar and send email'
        operations: [
          {
            ...
            agentParameters: [
              {
                ...
                schema: {
                  type: 'string'
                  description: 'The calendar ID. Use "Calendar" for the default calendar.'
                  required: false
                }
              }
              {
                ...
                schema: {
                  type: 'string'
                  format: 'date-time'
                  description: 'Start date-time for the calendar view (ISO 8601 format).'
                  required: true
                }
              }
              {
                ...
                schema: {
                  type: 'string'
                  format: 'date-time'
                  description: 'End date-time for the calendar view (ISO 8601 format).'
                  required: true
                }
              }
            ]
