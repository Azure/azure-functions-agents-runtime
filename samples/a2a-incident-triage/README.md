...
The simple profile is intentionally small. It does not support streaming, Tasks,
...
> [!IMPORTANT]
> Use Azure Functions Core Tools **4.14.0 or later** (`func --version`).
> On Windows, Core Tools 4.10.0 stops during indexing with
> `py exited with code -1073741819 (0xC0000005)`. The cause is a Protobuf
> conflict with the worker ([#211](https://github.com/Azure/azure-functions-agents-runtime/issues/211)).
> To update: `npm install -g azure-functions-core-tools@4`. Core Tools 5.x
> previews are a different product line and are not tested with this sample.
...
2. Create a separate client environment. It contains only the published MAF A2A
   ...
   compatibility PR and is not part of this sample.
