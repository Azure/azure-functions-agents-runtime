...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Inspect one raw A2A 1.0 JSON-RPC exchange with the incident-triage agent."
    )
    parser.add_argument(
        "prompt",
        nargs="?",
    ...
    parser.add_argument(
        "--return-immediately",
        action="store_true",
        help="Set returnImmediately=true (it has no effect for the direct Message profile).",
    )
    return parser
