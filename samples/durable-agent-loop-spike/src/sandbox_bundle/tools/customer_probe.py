...
def customer_probe(message: str, repeat: int = 1) -> dict[str, object]:
    """Return a deterministic customer-tool marker from the sandbox process."""
    if not isinstance(message, str) or not 1 <= len(message) <= 256:
        raise ValueError("message must contain between 1 and 256 characters")
    if isinstance(repeat, bool) or not 1 <= repeat <= 20:
