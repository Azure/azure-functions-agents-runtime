...
| `azure_functions_agents/config/schema.py` | Defines the Pydantic models for raw, global, and merged config, including independent object-only chat and workflow Sub Agent grants and explicit native A2A endpoint configuration. | `AgentSpec`, `GlobalConfig`, `ResolvedAgent`, `TriggerSpec`, `BuiltinEndpointsConfig`, `A2AConfig`, `SubagentRef`, `WorkflowConfig`, `WorkflowSubagentRef` |
...
| `azure_functions_agents/config/validation.py` | Post-merge sanity checks for resolved agents, including rejecting unknown/duplicate/self references in both independent Sub Agent grants against the app-wide slug index and validating trusted A2A publication URLs. | `validate_resolved_agent()`, `validate_subagent_references()`, `validate_workflow_subagent_references()` |
...
| `azure_functions_agents/registration/endpoints.py` | Registers debug chat UI, REST chat, SSE streaming, and MCP tools for agents with built-in endpoints; delegates explicit A2A declarations to the optional A2A registration module. | `register_builtin_endpoints()` |
| `azure_functions_agents/registration/a2a.py` | Registers the experimental P3 per-agent Agent Card and A2A 1.0 JSON-RPC routes. It uses the public MAF hosting adapter and SDK request-handler/routes while reusing the existing non-streaming runner, auth policy, workflow integration, and runtime sessions. | `register_a2a_endpoints()` |
| `azure_functions_agents/registration/_auth.py` | Enforces inbound endpoint auth: maps the configured `auth.mode` to a Functions `AuthLevel` (API key / anonymous) and enforces Entra ID identity by trusting the platform-validated Easy Auth `x-ms-client-principal` header (never validating tokens in-app), with optional tenant/audience/client-id allowlists. It also derives the authorized caller scope used to isolate A2A conversation contexts. Because `entra` routes are anonymous, the header is trusted only with non-spoofable evidence Easy Auth is enforced (`WEBSITE_AUTH_ENABLED` / `AZURE_FUNCTIONS_AGENTS_ENTRA_EASY_AUTH`); fails closed (401) otherwise. | `resolve_endpoint_auth_level()`, `authorize_entra_request()`, `resolve_authorized_request_scope()` |
...
The P3 A2A handler follows the same handoff rather than invoking a MAF agent
directly. The public `AgentA2AAdapter` converts the supported A2A text Message
into runner input and converts the runner's text output back into A2A Parts;
`_run_builtin_agent()` preserves the normal tool, skill, timeout, session,
policy, and workflow behavior. The SDK's public `RequestHandler` and
`create_jsonrpc_routes` own native A2A 1.0 dispatch and correlated error
envelopes. P3 deliberately returns only a direct Message: no Task store,
streaming path, REST binding, or distributed executor exists.
...
- **Built-in endpoints:** endpoint registration is separate from trigger registration so trigger bindings stay focused. The optional `registration/a2a.py` module is imported only for an explicit A2A declaration, keeping the base installation independent of A2A packages.
