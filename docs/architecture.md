...
Each workflow tool node runs as a normal Activity Function and remains subject to the hosting plan's Function timeout. Long work must be split into bounded Activities; long waits use Durable timers. Non-HTTP trigger sessions are ephemeral, so their session-scoped management tools are not available on a later agent turn. Such workflows use a terminal Activity to deliver results to a queue, database, webhook, notification service, or other domain sink, while operators use Durable Functions or Durable Task Scheduler surfaces for status and control.
