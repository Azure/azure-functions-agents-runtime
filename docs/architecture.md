...
| `azure_functions_agents/registration/_auth.py` | Enforces inbound endpoint auth: maps the configured `auth.mode` to a Functions `AuthLevel` (API key / anonymous) and validates Entra ID identity (Easy Auth `x-ms-client-principal` header or a bearer JWT via JWKS) with optional tenant/audience/client-id allowlists. | `resolve_endpoint_auth_level()`, `authorize_entra_request()` |
