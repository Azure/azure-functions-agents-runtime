...
| `azure_functions_agents/session_state/store.py` | Async Table-backed session state store with session/run CRUD, ETag/CAS, session-scoped and distinct owner-scoped idempotency admission EGTs, terminal adoption, conditional result eviction, tombstoning, and bounded service-timestamped scans. It remains the controller's only durable state writer. | `AzureTableSessionStateStore`, `SessionStateStore`, `admit_run()`, `admit_new_session_run()`, `adopt_terminal_run()`, `tombstone_session()` |
...
| `azure_functions_agents/controller/{budget,http,streaming,reconciler}.py` | Provider-neutral ACA controller: one setup/wall clock, sync/LRO response mapping, replayable SSE, and bounded Table/platform lifecycle reconciliation. Registration adapts these outputs to Functions HTTP/timer bindings. | `RequestBudget`, `submit_run()`, `render_events()`, `SessionReconciler` |
...
read/cancel; URL identifiers are not authorization. A plain timer reconciles
Table rows against provider inventory, retains readable terminal status after
result eviction, and re-drives deleting candidates. The production capability
