...
| `azure_functions_agents/config/validation.py` | Post-merge sanity checks for resolved agents, including rejecting unknown/duplicate/self `subagents:` references against the app-wide slug index; also enforces FRD 0008's `session_runtime` startup validation matrix (10 of 13 rows active — rows 6 and 7 are superseded since session state always reuses `AzureWebJobsStorage` with no dedicated-account or Shared-Key gate, and row 11 is structurally unrepresentable since retention only nests inside aca_sandbox) as a hard, typed gate before any other cross-agent validation runs — a no-op unless `session_runtime` is configured, and always a hard failure whenever `aca_sandbox` is configured today (no execution backend exists yet). Deliberately self-contained: it raises its own `ValueError`s rather than importing `execution/*`, so `config/` gains no new dependency edge on the execution package. | `validate_resolved_agent()`, `validate_subagent_references()`, `validate_session_runtime()` |
...
| `azure_functions_agents/registration/_handlers.py` | Builds the callable closures that turn incoming trigger data or HTTP bodies into runner prompts, threading the `AgentCatalog` through to the runner and combining tool-error heuristics with explicit delegate-error accounting; delegates non-HTTP binding payloads to the trigger serializer. For a bound persistent-session runtime, HTTP closures resolve the existing typed owner principal before backend construction. | `make_agent_handler()`, `make_http_agent_handler()`, `build_sandbox_tools_for_session()`, `_total_tool_error_count()` |
...
| `azure_functions_agents/session_state/session_models.py` | Defines FRD 0008 P3a's immutable Azure-SDK-neutral durable contract for the `AzureFunctionsAgentsSessions` table: owner partitions, session/run/hashed-idempotency rows, schema-v1 serialization, generation transition rules, per-status `active_run_id` lifecycle invariants, bounded snapshot-ID JSON, and the same-partition admission invariant required for a later entity-group transaction. `state_store_fingerprint` is validated here as the same label-safe `s1-<52 base32>` shape (P3b/Decision 114 computes its actual bytes). P3b (below) owns Table I/O/CAS/EGT; P4b now captures/verifies the live sandbox manifest (including `state_store_fingerprint`) against this row; live region, current-storage-epoch freshness, and generation interpretation before serving remain P5a, and reaper reconciliation remains P6. Renamed from `models.py` (P3b, mechanical `git mv`) to satisfy the repo-wide unique-intent-revealing-module-name convention shared with `transport/transport_models.py`. | `DurableSessionRecord`, `DurableRunRecord`, `DurableIdempotencyRecord`, `AdmissionRecords`, `encode_snapshot_ids()`, `validate_state_store_fingerprint()` |
...
| `azure_functions_agents/session_state/store.py` | FRD 0008 P3b's async Table-backed session state store: the `SessionStateStore` Protocol plus `AzureTableSessionStateStore`, its one real implementation. Provides idempotent table creation; session/run CRUD with ETag/CAS; one-active-run admission via an Azure Table entity-group transaction (idempotency dedup before the active-run check, typed conflicts for both); idempotent terminal-run adoption that releases the session's active slot only when still owned; conditional tombstoning that preserves historical fields; and a bounded, continuation-token-aware raw entity query for future reaper use (no reaper policy here). Every Azure SDK failure maps to a narrow typed error from `errors.py`, verified against a real Azurite Table service for every CAS/EGT/race claim. Live sandbox-manifest capture/verification against this store's rows is P4b (`controller/package.py`); live region/current-epoch cross-verification before serving is P5a; reconciliation/storage-epoch reset is P6. | `AzureTableSessionStateStore`, `SessionStateStore`, `admit_run()`, `adopt_terminal_run()`, `tombstone_session()` |
...
| `azure_functions_agents/execution/*` | Backend-neutral run-lifecycle seam that registration binds into: `backend.py` defines the exact four-method `AgentExecutionBackend` Protocol plus request/status/event dataclasses; `binding.py`/`compat.py` adapt registration's per-agent config and legacy call shape into that seam; `in_lang_worker.py` is the unchanged default implementation; `aca_sandbox.py` activates owner-scoped sessions and maps the same four methods onto `run_control.py`'s journal verbs; `setup_budget.py` supplies one cross-operation setup deadline; `factory.py` selects the sandbox backend only when an app-scoped session-runtime binding is present, and `unavailable.py` remains the fail-closed placeholder for a configured-but-unbound sandbox runtime. | `create_execution_backend()`, `AgentExecutionBackend`, `LanguageWorkerExecutionBackend`, `AcaSandboxExecutionBackend`, `SandboxRunControl` |
| `azure_functions_agents/controller/readiness.py` | Validates an owner-scoped durable session against live controller state, the configured Sandbox Group, and the sandbox manifest before a run is admitted; new sessions reserve their state row before provisioning and use a bounded manifest publication check. Attach and resume rely on the transport's own manifest handshake rather than repeating it. | `SessionRuntimeBinding`, `activate_session()`, `revalidate_before_submit()` |
| `azure_functions_agents/transport/*` | Controller-to-sandbox boundary. `transport_models.py` and `ports.py` own provider-neutral file/process/session projections plus the narrow `SandboxFileNotFoundError`/`SandboxFileOperationError` file-transport exception contract every `SandboxFileTransport` implementation raises; `manifest.py` strictly verifies controller-authoritative row inputs — including the non-secret `state_store_fingerprint` alongside owner/app/session/group/sandbox/generation/digest/protocol — against a direct data-plane manifest, and renders the same canonical JSON for both the controller-authored seed and the harness-authored live manifest; `aca_sdk.py` is the sole optional-preview SDK adapter for one customer-owned Sandbox Group, and is also the only module permitted to import the preview SDK's own exception types, translating them to the narrow contract at its file-operation boundary. It creates/deletes individual session sandboxes only; the execution seam reaches it exclusively through the `SandboxSessionProvider`/`SandboxSessionHandle` Protocols, never by importing the adapter. | `SandboxFileTransport`, `SandboxProcessTransport`, `SandboxSessionProvider`, `SandboxSessionHandle`, `SandboxFileNotFoundError`, `SandboxFileOperationError`, `verify_sandbox_manifest()`, `render_sandbox_manifest_binding()`, `AcaSandboxAdapter` |
...
8. `app.py`'s `_fail_on_duplicate_slugs()` builds the app-wide slug index and fails fast on collisions; `config/validation.py:validate_subagent_references()` then rejects unknown/duplicate/self `subagents:` references against that index.
