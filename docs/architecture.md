...
| `azure_functions_agents/workflows/activity.py` | Policy-aware Activity execution: strict validation of the persisted retry policy and per-attempt deadline, the `ok`/`failure` outcome envelope, the attempt deadline applied around the handler invocation, and the failure classification that decides whether Durable is asked to retry and whether a failure may be continued. Models read back from Durable history ignore unknown keys so a newer history still validates. | `invoke_policy_handler()`, `validate_activity_result()`, `failure_is_continuable()` |
