...
The A2A handler follows the same handoff rather than invoking a MAF agent
...
envelopes. The simple profile returns only a direct Message: no Task store,
streaming path, REST binding, or distributed executor exists. See [A2A server architecture](design/a2a-server.md).
