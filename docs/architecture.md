...
| `azure_functions_agents/journal_paths.py` | Owns the canonical `/var/lib/azurefunctions-agents-runtime` sandbox root and all child content, manifest, protocol, inbox, run, process, heartbeat, and checkpoint paths shared by controller and execution code. | `SANDBOX_ROOT_PATH`, `heartbeat_path()`, `process_path()` |
