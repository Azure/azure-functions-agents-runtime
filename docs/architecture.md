...
| `azure_functions_agents/registration/_auth.py` | Enforces inbound endpoint auth: maps the configured `auth.mode` to a Functions `AuthLevel` (API key / anonymous) and enforces Entra ID identity by trusting the platform-validated Easy Auth `x-ms-client-principal` header (never validating tokens in-app), with optional tenant/audience/client-id allowlists. Because `entra` routes are anonymous, the header is trusted only with non-spoofable evidence Easy Auth is enforced (`WEBSITE_AUTH_ENABLED` / `AZURE_FUNCTIONS_AGENTS_ENTRA_EASY_AUTH`); fails closed (401) otherwise. FRD 0008 P3a also exposes a dormant typed owner-principal seam: function/admin-key auth resolves only an app marker (never key bytes/name), while Easy Auth ownership additionally requires one stable `tid` + immutable `oid`. It is not wired into request execution yet. | `resolve_endpoint_auth_level()`, `authorize_entra_request()`, `resolve_owner_principal()` |
| `azure_functions_agents/session_state/identity.py` | Defines FRD 0008 P3a's pure, versioned Function App/slot and owner canonicalization: exact length-prefixed UTF-8 framing, `a1-`/`o1-` hashes, historical-version verification without eager migration, fail-closed platform identity resolution, typed owner resolution, server-minted IDs, and delimiter-safe durable row keys. It has no Azure SDK dependency and does not cross raw claims or credentials into execution. | `resolve_function_app_identity()`, `resolve_owner_context()`, `compute_app_hash()`, `compute_owner_hash()`, `owner_partition()` |
| `azure_functions_agents/session_state/models.py` | Defines FRD 0008 P3a's immutable Azure-SDK-neutral durable contract for the `AzureFunctionsAgentsSessions` table: owner partitions, session/run/hashed-idempotency rows, schema-v1 serialization, generation transition rules, bounded snapshot-ID JSON, and the same-partition admission invariant required for a later entity-group transaction. P3b owns Table I/O/CAS, P3c live-manifest/region/storage-epoch binding, and P3d reconciliation. | `DurableSessionRecord`, `DurableRunRecord`, `DurableIdempotencyRecord`, `AdmissionRecords`, `encode_snapshot_ids()` |
...
FRD 0008 P3a adds only the pure `session_state/` identity, key, and durable-row
contracts plus the non-wired `_auth.resolve_owner_principal()` seam. Registration
does not yet create owner contexts, open Azure Tables, or select the ACA backend,
and none of these values are added to `StartRunRequest` or the exact four-method
`AgentExecutionBackend` abstraction. Consequently, missing `aca_sandbox` still
selects `in_lang_worker` with unchanged behavior; P3a introduces no ACA SDK import
or live storage/transport operation.
