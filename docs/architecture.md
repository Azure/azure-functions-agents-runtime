...
- **Internal token accounting:** `runner.py` emits one `Agent token usage: {json}` INFO record through the shared `azure.functions.AgentRuntime` logger for each primary, delegated, or workflow-specialist MAF invocation attempt. `agent_name` is the resolved agent slug. Counts come only from the final MAF response; interrupted or provider-omitted usage is recorded as unavailable rather than estimated. Workflow/node ids group Durable retries of one logical workflow node; each log item remains one actual attempt. Session ids and endpoint-derived host metadata are not collected. Every record also carries the nullable provider, effective model/deployment, and publisher from the `InferenceTarget` returned beside that invocation's client. The runtime does not calculate pricing or revenue (FRD 0008).
