...
| `azure_functions_agents/config/validation.py` | Post-merge sanity checks for resolved agents, including rejecting unknown/duplicate/self `subagents:` references against the app-wide slug index; also enforces FRD 0008's `session_runtime` startup validation matrix (13 rows) as a hard, typed gate before any other cross-agent validation runs — a no-op unless `session_runtime` is configured, and always a hard failure for `provider: aca_sandbox` today (no execution backend exists yet). Deliberately self-contained: it raises its own `ValueError`s rather than importing `execution/*`, so `config/` gains no new dependency edge on the execution package. | `validate_resolved_agent()`, `validate_subagent_references()`, `validate_session_runtime()` |
...
- `execution/` answers **"once registration hands off a request, which backend actually runs it?"** — `in_process` today; `aca_sandbox` is a recognized-but-unavailable placeholder gated shut at startup, not at this seam (FRD 0008).
...
7. `config/validation.py:validate_session_runtime()` enforces FRD 0008's `session_runtime` startup validation matrix (13 rows) over the global config and resolved agents. A no-op unless `session_runtime` is configured at all — absence means `in_process` with no behavior change. A `provider: aca_sandbox` config always fails here with a typed diagnostic (the ACA backend does not exist yet); it never reaches trigger/endpoint registration or a first request.
