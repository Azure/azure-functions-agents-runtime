...
| `azure_functions_agents/session_state/identity.py` | Defines FRD 0008 P3a's pure, versioned Function App/slot and owner canonicalization: exact length-prefixed UTF-8 framing, portable `a1` identity (`subscription_id` + `site_name` + slot; no resource group / no SKU branches), `o1-` owner hashes, historical-version verification without eager migration, fail-closed platform identity resolution, typed owner resolution (function/admin key ⇒ app-owned shared sessions; Easy Auth ⇒ per-user), server-minted IDs, and delimiter-safe durable row keys. It has no Azure SDK dependency and does not cross raw claims, function keys, or credentials into execution. | `resolve_function_app_identity()`, `resolve_owner_context()`, `compute_app_hash()`, `compute_owner_hash()`, `owner_partition()` |
