...
- **Internal token accounting:** `runner.py` attempts one `Agent token usage: {json}` INFO record through the shared `azure.functions.AgentRuntime` logger for each primary, delegated, or workflow-specialist MAF invocation attempt. `agent_name` is the resolved agent slug. Counts come only from the final MAF response; interrupted or provider-omitted usage is recorded as unavailable rather than estimated. Usage is complete only when a successful response contains valid input, output, and total counts. Workflow/node ids group Durable retries of one logical workflow node; each log item remains one actual attempt. Session ids, invocation ids, schema versions, publisher, and endpoint-derived host metadata are not collected. Every record carries the nullable provider and effective model/deployment from the `InferenceTarget` returned beside that invocation's client. Emission is best-effort and non-fatal rather than a durable-delivery guarantee; the runtime does not calculate pricing or revenue (FRD 0008).
