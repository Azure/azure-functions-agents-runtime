...
flowchart LR
    ...
    E -->|"GlobalConfig + list of ResolvedAgent"| E1["config/validation.py<br/>validate_session_runtime"]
    E1 -->|"list of ResolvedAgent"| E2["app.py<br/>identity index<br/>fail-fast on duplicate slugs"]
    E2 -->|"ResolvedAgent + known_slugs"| F["config/validation.py<br/>validate_resolved_agent<br/>validate_subagent_references<br/>validate_workflow_subagent_references"]
...
Read left to right: files on disk become typed config, typed config becomes a `ResolvedAgent`, the app-wide session runtime is validated before registration, and each resolved agent is registered as Azure Functions bindings plus optional built-in endpoints. `project_composition.py` owns the shared read-only identity/reference/capability/catalog pass used by the Function App and remote runtimes. The identity-index and catalog nodes exist for multi-agent delegation (FRD 0007, Section 5 below): every agent's slug and capabilities are indexed and frozen *before* `H` mutates the `FunctionApp`, so chat-time and workflow subagent grants can resolve any specialist regardless of file order.

FRD 0009 adds an experimental, deployment-published execution choice after that
catalog is frozen. A complete eight-setting Foundry Hosted Agent (FHA) binding
is validated against the resolved application-content digest, its manifest-
carried canonical runtime projection, and Function App identity before
registration; an absent binding preserves the in-language-worker backend,
while a partial, stale, or invalid binding fails closed. This startup check
does not contact Foundry or Azure Table Storage: `FoundryResponsesRuntime`
opens both dependencies lazily only when an FHA request or supported trigger
fires.
...
- **Composition is two-pass and side-effect-free until pass 2.** `app.py`'s composition root builds the app-wide identity-slug index, fails fast on collisions, validates every `subagents:` and `workflows.subagents` reference, and freezes each agent's `ResolvedAgent` + `AgentCapabilities` into an immutable `AgentCatalog` — none of this touches the `FunctionApp` object. Only pass 2 (trigger/endpoint registration) mutates it (FRD 0007 §4.2).
...
- **Foundry Responses transport is also deferred.** The digest-gated
  `FoundryResponsesRuntime` is an app-scoped shell at startup; its Foundry
  transport and Table store are opened lazily at execution time, never by
  discovery, translation, or registration.
- **FHA deployment is outside startup.** A standalone, user-invoked bootstrap
  script stages and deploys the hosted agent, then publishes its complete
  binding. Startup and request handlers consume that verified binding only;
  neither creates, repairs, nor upgrades an FHA.
- **ACA transport is also deferred.** The optional `transport/aca_sdk.py` is constructed only
  through the app-scoped session-runtime binding at request/lifecycle time, never by discovery,
  config translation, or registration. The startup capability gate remains fail-closed until all
  runtime prerequisites are available.
- **ACA remains unchanged.** FHA and ACA session runtimes are mutually
  exclusive; enabling both fails startup rather than changing ACA lifecycle,
  identity, egress, or secret behavior.
...
| --- | --- | --- |
| `azure_functions_agents/app.py` | Top-level orchestrator that runs the startup pipeline and returns the configured app. It uses the shared project-composition pass before any `FunctionApp` mutation, then resolves the optional digest-gated FHA binding without opening its network dependencies. | `create_function_app()` |
| `azure_functions_agents/project_composition.py` | Runs the reusable read-only discovery, compose, identity/reference validation, capability filtering, and immutable-catalog pass used by normal Functions startup, the ACA harness, and FHA V0. | `compose_project()` |
...
| `azure_functions_agents/config/validation.py` | Post-merge sanity checks for resolved agents, including rejecting unknown/duplicate/self references in both independent Sub Agent grants against the app-wide slug index; resolves custom HTTP auth with the same pure helper as registration and rejects mismatched built-in/custom ACA policies before route mutation. It keeps the ACA capability gate fail-closed, including rejecting typed Dynamic Workflow and Workflow Sub Agent configuration under ACA; no authoring-schema change opens it. | `validate_resolved_agent()`, `validate_subagent_references()`, `validate_workflow_subagent_references()`, `validate_session_runtime()` |
...
| `azure_functions_agents/registration/_handlers.py` | Builds the callable closures that turn incoming trigger data or HTTP bodies into runner prompts, threading the `AgentCatalog` through to the runner and combining tool-error heuristics with explicit delegate-error accounting; delegates non-HTTP binding payloads to the trigger serializer. Bound session runtimes resolve the owner and adapt the provider-neutral LRO controller. FHA Service Bus handlers derive a stable delivery identity, remove volatile broker fields from the prompt, and poll within one configured lock budget. | `make_agent_handler()`, `make_http_agent_handler()`, `_make_fha_service_bus_handler()`, `build_sandbox_tools_for_session()`, `_total_tool_error_count()` |
...
| `azure_functions_agents/session_state/connection.py` | FRD 0008 P3b: resolves an Azure Table connection from `AzureWebJobsStorage` only (Decision #86) — connection string first, then identity-based `AzureWebJobsStorage__tableServiceUri` (+ `AzureWebJobsStorage__clientId`/`AZURE_CLIENT_ID`/bare `DefaultAzureCredential` precedence, matching `_blob_history.py`) — and fails closed with no dedicated-account or in-memory fallback. Derives the non-secret `s1-<52 base32>` state-store fingerprint (Decision 114) from only the resolved client's `.url`/`.account_name` (never the connection string/credential), and caches `TableServiceClient`s process-wide keyed by that fingerprint. Lazily imports `azure.data.tables` inside functions only, so importing this module never requires the `[aca_sandbox]` extra. | `resolve_table_connection_settings()`, `compute_state_store_fingerprint()`, `get_table_service_client()` |
| `azure_functions_agents/session_state/errors.py` | FRD 0008 P3b's typed, fail-closed store-error hierarchy (configuration, unavailable/throttled/auth, not-found, already-exists, ETag/generation/terminal-state conflicts, idempotency conflict, corrupt entity) — distinct from `session_models.py`'s pure `SessionStateContractError`, so store callers never need to catch shape-validation errors directly. | `SessionStateStoreError`, `ActiveRunConflictError`, `IdempotencyConflictError`, `ConcurrencyConflictError`, `GenerationConflictError` |
| `azure_functions_agents/session_state/store.py` | Async Table-backed session state store with session/run CRUD, ETag/CAS, idempotency admission EGTs, result eviction, app-scoped reconciliation cursors, and generic same-partition durable operations. `begin_provision_submit()` atomically claims owner idempotency, session, accepted run, and operation before a provider create; `admit_operation_run()` atomically advances an existing submit fence with its run/idempotency rows; `claim_operation_journal()` fences a launch lease so concurrent retries cannot both exec. Resume preserves phase while rotating token/lease; advance enforces per-kind forward transitions; complete and abort reject stale tokens. Owner keys remain until referenced work reaches its promised terminal retention. Admission rejects an active operation, and completed operations are retention-pruned. It remains the controller's only durable state writer. | `AzureTableSessionStateStore`, `SessionStateStore`, `begin_provision_submit()`, `admit_operation_run()`, `claim_operation_journal()`, `begin_operation()`, `resume_operation()`, `advance_operation()`, `complete_operation()`, `abort_operation()` |
| `azure_functions_agents/session_state` FHA provider mappings | Keeps `DurableProviderSessionBinding` and `DurableProviderRunMapping` in the existing owner partition. The run mapping records the `pending`/`submitting`/`bound`/terminal-or-indeterminate fence, private Response ID, and public-event watermark; store CAS operations prevent a duplicate submission. The provider session ID is exposed only as authenticated debug correlation metadata and never becomes runtime authority. | `create_provider_session_binding()`, `create_provider_run_mapping()`, `mark_provider_submission_issued()`, `bind_provider_response_id()` |
| `azure_functions_agents/journal_paths.py` | Owns the canonical `/var/lib/azurefunctions-agents-runtime` sandbox root and all child content, manifest, protocol, inbox, run, process, heartbeat, checkpoint, and launch-stderr-sidecar paths shared by controller and execution code. The per-run launch stderr sidecar is harness-written and controller-read: the harness redirects fd 2 into it only through run acceptance and then silences the fd (bounding its growth), and the controller reads it best-effort, size-bounded, and only on the indeterminate submission path. It is untrusted sandbox text — never parsed as a journal document. The marker prefix is not an authorship check, so a line is logged verbatim only when its prefix-stripped remainder is an exact member of the closed `ALLOWED_LAUNCH_DIAGNOSTICS` set of repo-authored messages; everything else, including a forged marker line carrying a secret, is reduced to line/byte metadata before it reaches any log. It persists under the run directory with the run's other journals. | `SANDBOX_ROOT_PATH`, `heartbeat_path()`, `process_path()`, `launch_stderr_path()`, `LAUNCH_DIAGNOSTIC_PREFIX`, `ALLOWED_LAUNCH_DIAGNOSTICS` |
| `azure_functions_agents/sandbox_runtime_limits.py` | Owns shared ACA lifecycle bounds and the complete per-sandbox auto-delete backstop arithmetic. | `MAX_RECONCILER_CADENCE_SECONDS`, `RECLAIM_SAFETY_GRACE_SECONDS`, `lifecycle_auto_delete_seconds()` |
...
| `azure_functions_agents/execution/*` | Backend-neutral run-lifecycle seam that registration binds into: `backend.py` defines the exact four-method `AgentExecutionBackend` Protocol plus request/status/event dataclasses; `in_lang_worker.py` remains unchanged; `aca_sandbox.py` retains its existing lifecycle; and `foundry_responses_execution_backend.py` maps the same four methods to stored Responses. A request-scoped setup budget reaches create polling without changing the four-method protocol. | `create_execution_backend()`, `AgentExecutionBackend`, `LanguageWorkerExecutionBackend`, `AcaSandboxExecutionBackend`, `FoundryResponsesExecutionBackend` |
| `azure_functions_agents/execution/foundry_responses_binding.py` | Parses the complete deployment-published, non-secret FHA binding; validates its Function App fingerprint, manifest-selected application-content digest, and canonical runtime-projection/wrapper digest; distinguishes absent (disabled) from partial/invalid (fail closed). | `resolve_foundry_responses_runtime_binding()`, `FoundryResponsesRuntimeBinding` |
| `azure_functions_agents/execution/foundry_responses_runtime.py` | Holds the validated FHA binding and app identity while lazily constructing one Foundry transport and one Table state-store binding per worker. | `FoundryResponsesRuntime.create()`, `get_transport()`, `get_state_store()` |
| `azure_functions_agents/execution/foundry_responses_execution_backend.py` | Implements `start_run`, `get_run`, `read_events`, and `cancel_run` against private Responses mappings. It creates new deterministic provider sessions at the active deployment version, reuses validated stored sessions across deployment updates, fences `pending` → `submitting` before provider create, polls stored Responses into deterministic terminal event snapshots rather than relying on stream TTL, and quarantines indeterminate submission or cancellation outcomes. Foundry may advance a reused session's effective version on the next Response. | `FoundryResponsesExecutionBackend`, `start_run()`, `get_run()`, `read_events()`, `cancel_run()` |
| `azure_functions_agents/execution/foundry_application_content.py` | Defines the platform-neutral, deployment-stable semantic manifest and SHA-256 digest shared by startup and standalone FHA bootstrap. It selects hosted catalog authoring, tools/skills, local tool dependencies, and explicit assets while excluding build/deployment artifacts; it is intentionally distinct from ACA package bytes/digests. | `build_application_content_manifest()`, `compute_application_content_digest()` |
| `azure_functions_agents/foundry_responses/*` | Compiles and gates the FHA V0 catalog, rejects substitutions and unsafe MCP configuration, serializes the canonical non-secret runtime projection, stages a secret-free source artifact with exact generated dependency pins, creates the resilient Responses entrypoint, and forces opaque owner/session `$HOME` history with run-idempotent markers. | `compile_fha_v0_project()`, `validate_fha_v0_catalog()`, `stage_fha_hosted_source()`, `create_fha_resilient_responses_host()`, `FhaHistoryFactory` |
| `azure_functions_agents/service_bus_delivery_identity.py` | Derives the non-secret Service Bus entity fingerprint from normalized namespace and queue, then derives the app/agent-scoped delivery key from that fingerprint and broker `sequence_number`. | `ServiceBusDeliveryIdentity.create()` |
| `azure_functions_agents/transport/foundry_responses.py` | Narrow async firewall around the Foundry project/Responses SDK. It creates caller-selected provider sessions at an explicit agent version, resolves retained sessions by private ID, and exposes create, retrieve, replay, and cancel for stored background Responses. The create request carries runtime-owned W3C trace headers only. | `FoundryResponsesAdapter`, `FoundrySessionCreateRequest`, `FoundryResponseCreateRequest`, `FoundryResponsesTransport` |
| `eng/scripts/bootstrap_foundry_responses_fha.py` | Standalone deployment boundary for the experimental FHA: it seals a manifest snapshot, stages local runtime source, verifies provenance, deploys an app-scoped hosted agent, resolves its instance/blueprint identities, applies or emits the exact hosted-observability RBAC handoff, smoke-tests, then publishes the eight non-secret settings and restarts the Function App. | `build_bootstrap_plan()`, `execute_bootstrap()` |
| `azure_functions_agents/controller/{budget,http,streaming,reconciler}.py` | Provider-neutral ACA controller: one setup/wall clock, sync/LRO response mapping, replayable SSE, and app-scoped lifecycle reconciliation. Timer scans are durable-cursor-bounded; request fast paths reuse targeted reconciliation. `reclaim_backing` covers active loss, idle teardown, and verified snapshot deletion; missing backing remains discoverable until referenced snapshots are removed. Labeled orphan snapshots are deleted before their proven backing so retry evidence survives failures. Terminal submit operations and intact terminal reclaim recovery remain fenced through remote idle-policy application and the final completion EGT, while page-local run absence never triggers ready-session reclaim. Reconciliation routes terminal successes through the persisted run agent slug's resolved output validator before adoption; invalid journal protocol terminalizes the matching run before quarantining the session and emits a redacted typed failure. | `RequestBudget`, `submit_run()`, `render_events()`, `SessionReconciler` |
| `azure_functions_agents/controller/readiness.py` | Validates an owner-scoped durable session against live controller state, the configured Sandbox Group, and the sandbox manifest before admission. `provision_new_session_submit()` reserves the first run before provider create, persists its sandbox target after discovery, and advances through lifecycle/content/manifest/journal phases under a stable label. Existing submissions use one `submit_run` fence through disarm, admission, journal acceptance, terminal rearm, and completion; terminal finalization requires the expected run ID before touching the active fence. A disarmed idle marker is valid only while its matching durable submit operation is active; an orphan marker fails closed. Attach/resume retain their transport handshake. | `SessionRuntimeBinding`, `activate_session()`, `provision_new_session_submit()`, `begin_submit_operation()`, `finalize_submit_operation()`, `revalidate_before_submit()` |
| `azure_functions_agents/harness/{atomic_commit,watchdog}.py` | Reusable stdlib sandbox-side durability primitives: whole-turn staging/rename/pointer commits and heartbeat/deadline supervision. They tolerate idle suspension but never own controller lifecycle-policy calls or credentials. | `AtomicCommitStore`, `Watchdog`, `freeze_harness_capabilities()` |
| `azure_functions_agents/transport/*` | Controller-to-sandbox boundary. Provider-neutral file/process/session ports include complete lifecycle policy, inventory, snapshot list/delete, and service metadata; the existing six file verbs remain unchanged. Provision requests may carry a stable operation label; `aca_sdk.py` discovers that label before retrying ambiguous creation, while remaining the only preview-SDK adapter and never being imported by controller/execution code. | `SandboxSessionProvider`, `SandboxSessionHandle`, `SandboxLifecyclePolicy`, `SandboxSummary`, `SandboxSnapshot`, `AcaSandboxAdapter` |
...
| `azure_functions_agents/controller/{sandbox_config,bootstrap_delivery}.py` | Builds the immutable app-scoped create profile (public disk, positive-provenance environment, one-shot bootstrap entrypoint, and create-time egress) and extends the existing provision-content fence with verified bootstrap delivery. | `SandboxCreateProfile`, `build_sandbox_create_profile()`, `deliver_content_and_bootstrap()` |
| `azure_functions_agents/harness/{bootstrap,__main__,journal_writer,delegation,sandbox_capabilities}.py` | Runs inside the sandbox. Bootstrap verifies content and ABI, freezes the exact capability map, writes `protocol.json`, then publishes readiness. The per-run entrypoint reconstructs the catalog, writes the controller-compatible journal, applies watchdog/atomic checkpointing, and records a semantic trace. | `prepare_sandbox()`, `JournalWriter`, `rebuild_agent_catalog()`, `register_sandbox_capabilities()` |
| `azure_functions_agents/egress/*` | Compiles one create-time Full-inspection policy from discovered destinations, static headers, and optional secret references. It orders Deny/Transform/Allow safely, caps rules, and keeps SDK translation outside this package. | `compile_egress_policy()`, `compile_mcp_headers()` |
...
| `azure_functions_agents/_observability.py` | Cross-cutting OpenTelemetry bootstrap and conventions: enables MAF `gen_ai` instrumentation and, when the optional `[monitor]` extra is installed, the Azure Monitor exporter; provides `af.*` span/attribute helpers and runtime-owned W3C trace-header injection without baggage; resolves the sensitive-data flag; emits minimal metrics; and controls third-party log noise. | `configure_observability()`, `start_span()`, `current_span()`, `get_current_operation_id()`, `FaultDomain`, `LifecycleStage`, `record_delegate_call()` |
...
- `execution/` answers **"once registration hands off a request, which backend actually runs it?"** — `in_lang_worker` remains the default; a complete verified FHA binding selects `foundry_responses`; and `aca_sandbox` remains a recognized-but-unavailable placeholder gated shut at startup (FRD 0008).
...
- `controller/` answers **"how does the controller get this session's code onto that sandbox, and how does it know the harness is really running it?"** — P4b captures a deterministic content archive, delivers it plus a seed through `transport/`'s file port, and later reads back the harness-published live manifest. It is not an execution backend either and, like `transport/`, is invoked lazily by a future ACA backend rather than at startup.
...
7. `config/validation.py:validate_session_runtime()` enforces FRD 0008's `session_runtime` startup validation matrix (10 of 13 rows active; rows 6 and 7 are superseded and row 11 is structurally unrepresentable) over the global config and resolved agents. A no-op unless `session_runtime` is configured at all — absence means the default `in_lang_worker` backend with no behavior change. A config with `aca_sandbox` present always fails here with a typed diagnostic (the ACA backend does not exist yet); it never reaches trigger/endpoint registration or a first request.
8. `app.py`'s `_fail_on_duplicate_slugs()` builds the app-wide slug index and fails fast on collisions; `config/validation.py` then rejects unknown/duplicate/self references independently for `subagents:` and `workflows.subagents` against that index.
9. `config/validation.py:validate_resolved_agent()` checks each merged object for missing triggers, bad MCP references, and similar config mistakes (an agent referenced only as a `subagents:` target is exempt from the trigger-or-`builtin_endpoints` requirement).
10. `registration/capabilities.py` converts name-based filters into concrete tool lists and skill paths, and fails fast on any `delegate_<slug>` tool-name collision.
11. `registration/catalog.py:build_catalog()` freezes every agent's `ResolvedAgent` + `AgentCapabilities` into one immutable `AgentCatalog` — this is the last step before anything touches the `FunctionApp`.
12. `app.py` inspects the optional FHA environment binding before selecting the
    composition path. With FHA enabled, the shared V0 compiler first rejects
    substitutions in raw catalog authoring, then produces the same allowed
    tool/Skill/remote-MCP/one-level-delegation catalog and canonical projection
    used by bootstrap and the hosted entrypoint. All eight binding values must
    validate, the manifest must reproduce the application-content digest and
    carried projection, and the binding must match the Function App identity.
    An absent binding keeps the default backend; a partial, stale, invalid,
    excluded-capability, or ACA-coexisting binding fails startup without
    calling Foundry.
13. `registration/triggers.py` and `registration/endpoints.py` mutate one `FunctionApp` instance until all agents are registered, threading the frozen `AgentCatalog` through so handler closures can build `delegate_<slug>` tools later, at request time.

That ordering matters because later modules assume earlier stages have already reduced free-form author input into typed, validated objects. For example, registration code does not re-parse YAML or front matter; it trusts `ResolvedAgent` and `AgentCapabilities`. Steps 6-12 are FRD 0007's "two-pass composition" (§4.2) plus the ACA/FHA startup gates: everything through step 12 is pure/side-effect-free (no `FunctionApp` mutation), and only step 13 is pass 2.
...
6. **Validate the `session_runtime` startup capability gate**
   - **Implemented by:** `src/azure_functions_agents/config/validation.py:validate_session_runtime()`
   - **Input:** `GlobalConfig`, `list[ResolvedAgent]`
   - **Output:** `None` (raises `ValueError` on the first matrix-row violation); the same `GlobalConfig` and `ResolvedAgent` values are otherwise unchanged
   - **Notes:** enforces FRD 0008's `session_runtime` startup validation matrix (10 of 13 rows active — rows 6 and 7 are superseded since session state always reuses `AzureWebJobsStorage` with no dedicated-account or Shared-Key gate, and row 11 is structurally unrepresentable since retention only nests inside aca_sandbox) — `retention` bounds and ordering (`auto_suspend_idle`/`reclaim_idle`), the `workflows.enabled` + `aca_sandbox` fail-closed gate (checked per agent via `ResolvedAgent.workflows`, not by cross-referencing agents against each other), and the auto-delete backstop invariant. This stage runs before the identity-index/`subagents:` pass below but is not itself a cross-agent check: it validates the global config (plus each agent's own `workflows` flag, independently) rather than relationships between agents. Backend selection has no explicit discriminant field: absence of `session_runtime` entirely, or presence without an `aca_sandbox` block, is a no-op — the default `in_lang_worker` backend applies with no behavior change; both cases now log an INFO-level line naming the selected (default) backend. Configuring `aca_sandbox` always fails here today, because no ACA backend exists yet; the failure is a startup-time `ValueError` with a typed message, never a first-request surprise. Deliberately self-contained: this module raises its own errors rather than importing `execution/*`, so `config/` gains no new dependency edge on the execution package.

7. **Build the app-wide identity index; validate `subagents:` references**
...
10. **Freeze the per-agent results into an immutable `AgentCatalog`**
...
11. **Create the Azure Functions app container**
...
12. **Register triggers and built-in endpoints (pass 2)**
...
Registration does not run the agent itself. Instead, `registration/_handlers.py` and `registration/endpoints.py` bind the agent's live configuration into an `AgentBinding` and call `execution/factory.py:create_execution_backend()`, then use only the execution lifecycle: `start_run()`, `read_events()`, and `get_run()`. Without an app-scoped session-runtime binding, the factory returns the unchanged `LanguageWorkerExecutionBackend`. With an ACA binding, request handlers resolve the typed owner principal and construct `AcaSandboxExecutionBackend`; its activation gate verifies the durable row, live state-store fingerprint, current content digest, configured group, and manifest before the file-backed journal accepts a run. The startup capability gate still blocks an incomplete sandbox runtime, and `UnavailableBackend` remains defense in depth for direct callers that request the named provider without a binding. `AgentBinding` holds the non-serializable per-agent inputs (instructions, concrete tools, skills, model, workflow client, subagents, and catalog); `StartRunRequest` carries only the per-turn prompt, session ID, authored timeout, and optional idempotency key. The `in_lang_worker` backend resolves `runner` lazily only when it starts execution. Non-streaming registration maps the terminal `RunResult` back to the existing `AgentResult` boundary; streaming registration renders the journaled `RunEvent` sequence back into the unchanged SSE chunks. A client disconnect does not cancel an `in_lang_worker` run: its journal is retained in memory through terminal state so a slow reader cannot lose events. For non-HTTP triggers, the closure delegates payload construction to `registration/_trigger_serialization.py`: native `to_dict()`/`model_dump()` contracts are used first, then public Azure Functions binding adapters, batch recursion, and byte encoding produce JSON-safe prompt data. HTTP handlers build their request-body JSON separately and do not use this serializer. The runner then asks the active `ClientManager` to build a chat client, builds any `delegate_<slug>` tools fresh for this request, and executes through the Microsoft Agent Framework (`src/azure_functions_agents/runner.py`, `src/azure_functions_agents/client_manager.py`).

### Experimental Foundry Hosted Agent Responses execution

FHA uses the same four-method execution seam without adding a front-matter or
`schema.py` setting. After the read-only discovery and typed composition stages
have frozen the catalog, `app.py` accepts only a complete,
environment-published `FoundryResponsesRuntimeBinding`. It validates the
manifest-selected application-content digest and Function App fingerprint,
rejects ACA coexistence, and then creates a lazy `FoundryResponsesRuntime`.
Registration remains the only Azure Functions-aware stage: it wires ordinary
HTTP routes and, in FHA mode, the one supported non-HTTP
`service_bus_queue_trigger`. It does not provision an FHA.

At invocation, `create_execution_backend()` selects
`FoundryResponsesExecutionBackend`. Its flow is:

1. `start_run()` retains the existing owner/session/idempotency and active-run
   admission authority in Azure Table Storage. For a new runtime session it
   deterministically derives a private provider session and creates it at the
   binding's exact hosted-agent version. For an existing runtime session it
   resolves the stored private provider session rather than deriving a new ID
   or migrating history. Foundry owns the effective version after resume and
   may advance it to the active deployment. It records each private run mapping
   through
   `pending` → `submitting` → `bound` before exposing the Response ID. The
   initial Responses create runs under one worker client span and carries only
   W3C `traceparent`/`tracestate`; Foundry AgentServer preserves that context for
   hosted MAF/model spans.
2. `get_run()` retrieves the stored Response and adopts a proven terminal
   projection into the durable run record. For an attached built-in
   `/chatstream` request, `read_events()` consumes the original provider reader,
   maps provider sequence `p` to public SSE ID `p+1`, and emits output-text
   deltas through the existing `delta` event. After disconnect, or for an async
   run, later readers use stored provider replay/retrieval for durable lifecycle
   and terminal snapshots; the hosted path does not replay earlier text deltas.
   It never resubmits work.
   `cancel_run()` retrieves, cancels, and polls only until a terminal outcome
   can be proven. An indeterminate submission or termination quarantines the
   durable session instead of retrying blindly.
3. The generated hosted entrypoint sets its staged app root, loads the canonical
   non-secret runtime projection, rebuilds the same FHA V0 catalog, and enables
   resilient Responses tasks. The normal MAF runner receives custom Python
   tools, Agent Skills, remote HTTP MCP, and one-level delegation plus forced
   opaque owner/session-scoped `$HOME` history. For an attached stream, one
   producer task maps MAF text updates into AgentServer output-text deltas; only
   after the MAF stream and history commit finish does the handler finalize the
   output item and checkpoint it. A pre-checkpoint crash may replay that whole
   turn and its effects at least once. System tools, workflows, nested
   delegation, local MCP, custom secrets, and ACA-like egress or `secretRef`
   behavior remain outside this spike.
4. For a Service Bus delivery, the handler uses an app/agent-scoped,
   non-secret namespace-and-queue fingerprint plus broker `sequence_number` as
   its stable idempotency input. It reattaches on redelivery and polls the
   known run to terminal under one explicit lock budget; an unsuccessful or
   exhausted delivery raises for normal retry/DLQ handling.

FHA resource creation, source staging, active-version validation, smoke
testing, and binding publication are deployment work performed by
`eng/scripts/bootstrap_foundry_responses_fha.py` after the normal Functions
deployment. After the exact version is active, bootstrap resolves the agent
instance and blueprint identities and ensures the two hosted-observability role
layers (`Reader` on the Foundry account and `Monitoring Metrics Publisher` on
the Function App's shared Application Insights resource). It applies only those
roles when authorized or emits an admin handoff and stops before binding
publication. These steps are intentionally outside app startup and request
execution. ACA remains a separate and unchanged runtime path.

FRD 0008 P3a adds the pure `session_state/` identity, key, and durable-row
contracts plus the non-wired `_auth.resolve_owner_principal()` seam. FRD 0008 P3b
adds the Table-backed I/O capability on top — `session_state/connection.py`
(`AzureWebJobsStorage` resolution + the non-secret `s1-` state-store fingerprint)
and `session_state/store.py` (`AzureTableSessionStateStore`: CRUD, ETag/CAS,
one-active-run admission EGT, idempotent terminal adoption, tombstoning) — verified
against a real Azurite Table service. Neither phase wires anything into
registration/execution yet: registration does not create owner contexts, does not
call the store, and does not select the ACA backend, and none of these values are
added to `StartRunRequest` or the exact four-method `AgentExecutionBackend`
abstraction. Consequently, missing `aca_sandbox` still selects `in_lang_worker`
with unchanged behavior; P3a/P3b introduce no ACA SDK import, and `azure.data.tables`
is only ever imported lazily inside `session_state` functions (never at module
...
For a bound ACA request, registration resolves endpoint auth and the owner,
anchors one request budget, then constructs the four-method backend. The
...
terminal status after result eviction and re-drives deleting candidates. The production capability
gate remains closed until the deferred bootstrap image/create source exists.
...
- [`docs/aca-sandbox-session-runtime.md`](aca-sandbox-session-runtime.md) — experimental sandbox bootstrap, journal, egress, and identity contract
