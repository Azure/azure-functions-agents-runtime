...
- **Hybrid tool execution is a private spike.** When its explicit environment gate is present,
  ...
  polling its LRO, and closes clients. If the bounded handle initiation fails before acceptance,
  it immediately attempts separately bounded exact-ID provider deletion; confirmed fallback
  cleanup never invalidates completed output. Terminal lifecycle and the app-scoped reaper remain
  armed if both seams fail. This path is distinct from the conversation-scoped
