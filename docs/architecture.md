...
| `azure_functions_agents/session_state/store.py` | Async Table-backed session state store with session/run CRUD, ETag/CAS, idempotency admission EGTs, result eviction, app-scoped reconciliation cursors, and generic same-partition durable operations. `begin_provision_submit()` atomically claims owner idempotency, session, accepted run, and operation before a provider create; ambiguous acknowledgements are confirmed by point-reading that exact row set. `admit_operation_run()` does the equivalent for an existing session. `claim_operation_journal()` fences launch against pre-launch cancellation; `cancel_prelaunch_submit()` terminalizes an accepted new- or existing-session run and advances its matching operation to rearm, while final fenced cleanup clears the operation and active-run slot together. Resume preserves phase while rotating token/lease; advance enforces per-kind forward transitions; complete and abort reject stale tokens. Owner keys remain until referenced work reaches its promised terminal retention. Admission rejects an active operation, and completed operations are retention-pruned. It remains the controller's only durable state writer. | `AzureTableSessionStateStore`, `SessionStateStore`, `begin_provision_submit()`, `confirm_provision_submit()`, `admit_operation_run()`, `confirm_operation_run_admission()`, `cancel_prelaunch_submit()`, `claim_operation_journal()`, `begin_operation()`, `resume_operation()`, `advance_operation()`, `complete_operation()`, `abort_operation()` |
