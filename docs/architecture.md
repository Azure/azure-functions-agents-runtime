...
| `azure_functions_agents/workflows/retry.py` | Defines the Azure-independent policy-aware Activity outcome contract and shared retry/continue/fail decision. Strict Pydantic models validate untrusted persisted policy fields only at the Activity boundary; replay-sensitive orchestration continues to use plain JSON/TypedDict values. | `PolicyActivityInputModel`, `decide_retry()`, `validate_activity_result()` |
