...
| `azure_functions_agents/execution/*` | Backend-neutral run-lifecycle seam that registration binds into: `backend.py` defines the exact four-method `AgentExecutionBackend` Protocol plus request/status/event dataclasses; `in_lang_worker.py` remains unchanged by ACA budgets; `aca_sandbox.py` activates owner-scoped sessions, applies owner/session idempotency, and maps the same four methods onto journal verbs. One anchored 90-second typed setup budget reaches reconciliation and create polling without changing the four-method protocol; `provision_submit` uses a 120-second durable lease. | `create_execution_backend()`, `AgentExecutionBackend`, `LanguageWorkerExecutionBackend`, `AcaSandboxExecutionBackend`, `SandboxRunControl` |
