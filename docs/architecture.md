...
| `azure_functions_agents/journal_paths.py` | Owns the canonical `/var/lib/azurefunctions-agents-runtime` sandbox root and all child content, manifest, protocol, inbox, run, process, heartbeat, checkpoint, and launch-stderr-sidecar paths shared by controller and execution code. The per-run launch stderr sidecar is harness-written and controller-read: the harness redirects fd 2 into it only through run acceptance and then silences the fd (bounding its growth), and the controller reads it best-effort, size-bounded, and only on the indeterminate submission path. It is untrusted sandbox text — never parsed as a journal document, and allow-listed to repo-authored `LAUNCH_DIAGNOSTIC_PREFIX` marker lines (all other content logged as metadata only) before it reaches any log. It persists under the run directory with the run's other journals. | `SANDBOX_ROOT_PATH`, `heartbeat_path()`, `process_path()`, `launch_stderr_path()`, `LAUNCH_DIAGNOSTIC_PREFIX` |
