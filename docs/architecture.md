...
| `azure_functions_agents/workflows/activity.py` | Policy-aware Activity execution: strict validation of the persisted retry policy and per-attempt deadline, the `ok`/`failure` outcome envelope, the attempt deadline applied around the handler invocation, the failure classification that decides whether Durable is asked to retry and whether a failure may be continued, and the `workflow.task.activity` span each delivery records (including deliveries denied before the handler runs). Models read back from Durable history ignore unknown keys so a newer history still validates. | `invoke_policy_handler()`, `validate_activity_result()`, `failure_is_continuable()`, `early_policy_outcome_with_telemetry()` |
...
| `azure_functions_agents/_observability.py` | Cross-cutting OpenTelemetry bootstrap and conventions: enables MAF `gen_ai` instrumentation and, when the optional `[monitor]` extra is installed, the Azure Monitor exporter, provides the `af.*` span/attribute helpers (fault domain, lifecycle stage), the resolved sensitive-data flag from `ENABLE_SENSITIVE_DATA`, minimal dynamic-session, delegate-call, and workflow-task metrics, and third-party log-noise control. Workflow-task counters carry only low-cardinality dimensions; identifiers stay on the span. | `configure_observability()`, `start_span()`, `current_span()`, `FaultDomain`, `LifecycleStage`, `record_delegate_call()`, `workflow_task_activity_telemetry()` |
...
Progress is published as a structured `custom_status` snapshot (logical node
states plus per-instance state): `schema_version: 2` for a plan that froze no
task execution policy, and `schema_version: 3` — the same snapshot plus the
frozen attempt budget, continued failures, and `retry_driver: "durable"` — for
one that did. The four controlled
