...
| `azure_functions_agents/client_manager.py` | Defines the pluggable inference-client abstraction, immutable inference-target metadata, and the default MAF-backed implementation for Microsoft Foundry, Azure OpenAI, OpenAI, and GitHub Models. GitHub Models reuses MAF's OpenAI-compatible client with a configurable base URL; only its dedicated token participates in auto-detection. | `ClientManager`, `InferenceTarget`, `get_client_manager()`, `set_client_manager()` |
