...
- A **parent/request cancellation** (`asyncio.CancelledError`) is *not* caught by the handler at all — as a `BaseException`, it passes through the `except` clauses untouched, so cancelling the coordinator's run (client disconnect, host shutdown, coordinator-level timeout) propagates into any in-flight specialist call and aborts it too, rather than being swallowed as a "recoverable" tool error. Because the specialist runs through a plain, non-streaming `agent.run()` call, MAF's own OTel spans for that call close deterministically on cancellation too — no explicit finalize step is needed (see FRD 0006 §5 Decision #20).
