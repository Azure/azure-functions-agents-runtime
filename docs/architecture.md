...
| `azure_functions_agents/config/validation.py` | Post-merge sanity checks for resolved agents, including rejecting unknown/duplicate/self `subagents:` references against the app-wide slug index; also enforces FRD 0008's `session_runtime` startup validation matrix (12 rows still active out of the FRD's original 13 — only row 11, "retention set for a provider other than `aca_sandbox`", became structurally unrepresentable once `provider` was removed in favor of presence-based backend selection; row 5, "missing/empty `sandbox_group_resource_id`", remains fully enforced, now entirely via `AcaSandboxConfig`'s own Pydantic-level required field rather than bespoke code in this module) as a hard, typed gate before any other cross-agent validation runs — a no-op unless `session_runtime` is configured, and always a hard failure whenever `aca_sandbox` is configured today (no execution backend exists yet). Deliberately self-contained: it raises its own `ValueError`s rather than importing `execution/*`, so `config/` gains no new dependency edge on the execution package. | `validate_resolved_agent()`, `validate_subagent_references()`, `validate_session_runtime()` |
...
7. `config/validation.py:validate_session_runtime()` enforces FRD 0008's `session_runtime` startup validation matrix (12 rows still active out of the FRD's original 13 — only row 11 became structurally unrepresentable) over the global config and resolved agents. A no-op unless `session_runtime` is configured at all — absence means the default `in_lang_worker` backend with no behavior change. A config with `aca_sandbox` present always fails here with a typed diagnostic (the ACA backend does not exist yet); it never reaches trigger/endpoint registration or a first request.
...
6. **Validate the `session_runtime` startup capability gate**
   ...
   - **Notes:** enforces FRD 0008's `session_runtime` startup validation matrix (12 rows still active out of the FRD's original 13 — only row 11, "retention set for a provider other than `aca_sandbox`", became structurally unrepresentable once `provider` was removed; row 5, "missing/empty `sandbox_group_resource_id`", remains fully enforced, now entirely via `AcaSandboxConfig`'s Pydantic-level required field) — `retention` bounds and ordering (`auto_suspend_idle`/`reclaim_idle`), the dropped-field rejections (`max_run_seconds`, `region`, `disk`, `content_package`), the `workflows.enabled` + `aca_sandbox` fail-closed gate (checked per agent via `ResolvedAgent.workflows`, not by cross-referencing agents against each other), and the auto-delete backstop invariant. This stage runs before the identity-index/`subagents:` pass below but is not itself a cross-agent check: it validates the global config (plus each agent's own `workflows` flag, independently) rather than relationships between agents. There is no explicit backend-selector field: absence of `session_runtime` entirely, or presence without an `aca_sandbox` block, is a no-op — the default `in_lang_worker` backend applies with no behavior change. Configuring `aca_sandbox` always fails here today, because no ACA backend exists yet; the failure is a startup-time `ValueError` with a typed message, never a first-request surprise. Deliberately self-contained: this module raises its own errors rather than importing `execution/*`, so `config/` gains no new dependency edge on the execution package.
