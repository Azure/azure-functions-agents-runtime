...
flowchart LR
    ...
    H -.->|"same durable-loop gate"| R["experimental/durable_chat_*<br/>Hosted UI, journal, and diagnostics"]
...
- **Hybrid tool execution is a private spike.** When its explicit environment gate is present,
  ...
  At request time `experimental/hybrid_tools.py` creates one fresh ACA Sandbox, disables active
  auto-suspend before packaging, captures either the full app root or an explicit private bundle,
  and verifies the package digest inside the sandbox before importing customer code. Exact
  fail-closed stub identities route through middleware to the sandbox; lazy MCP functions call
  next and remain in the worker. After streaming or non-streaming completion, the lease drains,
  marks executor shutdown, arms terminal lifecycle, initiates server-side deletion without
...
- **The Durable Agent Loop chat UI is an opt-in private presentation path, not
  a second chat gate.** The same
  ...
  observations are best effort and cannot affect execution.
...
observations flow through a bounded Blob-backed journal outside Durable
orchestration history. The browser replays a snapshot or contiguous SSE deltas
and persists each projection with its cursor atomically. Journal failure
degrades the UI to authoritative status/result behavior; it never turns a
successful model/tool execution into a failure.

The inspector keeps configured Sandbox Group metadata separate from actual
local-tool sandbox observations, which are historical rather than live health
claims. Its DTS and Application Insights actions are credential-free,
request-frozen diagnostics; see
[the Durable Agent Loop chat UI guide](durable-agent-loop-chat-ui.md) for
retention, authentication, and portal limitations.

## 3. Module map

| Package/module | Role | Key entry points |
| --- | --- | --- |
| `azure_functions_agents/app.py` | Top-level orchestrator that runs the startup pipeline and returns the configured app; owns the two-pass composition root (FRD 0007 §4.2) — builds the app-wide identity-slug index, fails fast on duplicate slugs, and freezes every agent's validated `ResolvedAgent` + `AgentCapabilities` into an immutable `AgentCatalog` before any `FunctionApp` mutation. Its ACA reconciler timer composes a fixed 240-second application deadline around a whole pass (below the common five-minute host timeout); expiration cancels awaited provider work, logs a redacted error, fails the invocation, and lets the next cadence retry. Request-path targeted reconciliation is outside this timer-only boundary. | `create_function_app()`, `_fail_on_duplicate_slugs()`, `_run_deployed_reconciler_timer_pass()` |
...
| `azure_functions_agents/config/schema.py` | Defines the Pydantic models for raw, global, and merged config, including independent object-only chat and workflow Sub Agent grants. ACA selection requires an app-wide Sandbox Group resource ID plus its normalized authored region. | `AgentSpec`, `GlobalConfig`, `AcaSandboxConfig`, `ResolvedAgent`, `TriggerSpec`, `BuiltinEndpointsConfig`, `SubagentRef`, `WorkflowConfig`, `WorkflowSubagentRef` |
...
| `azure_functions_agents/experimental/durable_loop_activities.py` | Activity-side one-step MAF adapter plus immutable content, compaction, resume, and background-response seams. `BlobDurableContentStore` uses commit-once block blobs and integrity refs; `MafOneStepModelProvider` constructs a fresh Agent with no session/history providers, forces `store=False` plus encrypted-reasoning capture, rejects continuation fields, and disables local function invocation before one inference. For an opted-in foreground chat run, it uses `Agent.run(stream=True)` to observe real text before applying the unchanged terminal response parser/validation. | `BlobDurableContentStore`, `MafOneStepModelProvider`, `BackgroundModelProvider`, `DeterministicContextCompactor` |
| `azure_functions_agents/experimental/durable_loop_apim.py` | APIM-backed foreground/background Responses provider. It owns bounded retry classification, start/poll/cancel receipts, external provider-ID/body persistence, fixed control paths, backend affinity hashes, encrypted-reasoning parsing, and terminal-status mapping. The private demo fault path adds an attempt-local correlation/fault header so APIM can return one real 429 before a bounded retry; V3 makes that exact injected 429 escape its activity before provider retry, while the recovery activity and V1/V2 retain normal bounded provider retries. The header never mutates shared client defaults or enters diagnostics. | `ApimMafResponsesProvider`, `ApimResponsesTransport` |
...
| `azure_functions_agents/experimental/durable_loop_execution.py` | Worker composition for the trusted model/MCP plane and isolated ACA plane. It reconstructs and verifies the frozen catalog on each worker before dispatch, and records that remote MCP calls have no local sandbox when an opted-in chat observer is present. | `DurableExecutionPlaneRouter`, `build_durable_execution_plane()` |
...
| `azure_functions_agents/experimental/durable_loop_sandbox.py` | ACA single-call waves, optional retained-session ownership, capacity admission, exact inventory-loss recovery, immutable workspace restore/export, request-bound receipts, explicit cleanup, ambiguous-write truth, and owner-validated read-only retained-sandbox inspection for the private demo surface. Its optional chat observer preserves actual local sandbox IDs and timestamped lifecycle observations, including replacement and deletion, without delaying cleanup. | `DurableAcaSandboxLane`, `DurableSandboxCapacityCoordinator` |
...
| `azure_functions_agents/session_state/store.py` | Async Table-backed session state store with session/run CRUD, ETag/CAS, idempotency admission EGTs, result eviction, app-scoped reconciliation cursors, and generic same-partition durable operations. `begin_provision_submit()` atomically claims owner idempotency, session, accepted run, and operation before a provider create; ambiguous acknowledgements are confirmed by point-reading that exact row set. `admit_operation_run()` does the equivalent for an existing session. `claim_operation_journal()` fences launch against pre-launch cancellation; `cancel_prelaunch_submit()` terminalizes an accepted new- or existing-session run and advances its matching operation to rearm, while final fenced cleanup clears the operation and active-run slot together. Terminal adoption can atomically extend session expiry to the shared result-availability floor while releasing the active slot. Resume preserves phase while rotating token/lease; advance enforces per-kind forward transitions; complete and abort reject stale tokens. Owner keys remain until referenced work reaches its promised terminal retention. Admission rejects an active operation, and completed operations are retention-pruned. It remains the controller's only durable state writer. | `AzureTableSessionStateStore`, `SessionStateStore`, `begin_provision_submit()`, `confirm_provision_submit()`, `admit_operation_run()`, `confirm_operation_run_admission()`, `cancel_prelaunch_submit()`, `claim_operation_journal()`, `begin_operation()`, `resume_operation()`, `advance_operation()`, `complete_operation()`, `abort_operation()` |
...
| `azure_functions_agents/sandbox_runtime_limits.py` | Owns shared ACA lifecycle bounds, the 300-second result hold, and the complete per-sandbox auto-delete backstop arithmetic. | `MAX_RECONCILER_CADENCE_SECONDS`, `RECLAIM_SAFETY_GRACE_SECONDS`, `RESULT_HOLD_SECONDS`, `lifecycle_auto_delete_seconds()` |
...
| `azure_functions_agents/execution/*` | Backend-neutral run-lifecycle seam that registration binds into: `backend.py` defines the exact four-method `AgentExecutionBackend` Protocol plus request/status/event dataclasses; `in_lang_worker.py` remains unchanged by ACA budgets; `aca_sandbox.py` applies owner/session idempotency and maps the same four methods onto durable admission plus journal verbs. One anchored 90-second typed setup budget reaches reconciliation and create polling without changing the protocol, and every durable operation uses a sliding 120-second lease. ACA management is Table-first while a submit operation is provisioning, then activates the sandbox only after the journal-launch claim. An indeterminate launch preserves the committed management handle, and every management method propagates runtime-owned activation errors rather than raw transport failures. Public `phase` distinguishes provisioning, executing, settling, and terminal progress without adding a durable run state. | `create_execution_backend()`, `AgentExecutionBackend`, `LanguageWorkerExecutionBackend`, `AcaSandboxExecutionBackend`, `SandboxRunControl` |
| `azure_functions_agents/execution/aca_composition.py` | Internal ACA composition facade used by application registration and non-HTTP qualification callers. It builds resolved bindings, catalog, sandbox create profile, and session runtime without registering Azure Functions routes; transport SDK imports remain confined to `transport/aca_sdk.py`. | `compose_aca_execution()`, `compose_aca_application()`, `AcaExecutionComposition` |
| `azure_functions_agents/controller/{budget,http,streaming,reconciler}.py` | Provider-neutral ACA controller: one setup/wall clock, durable-admission-aware sync/LRO response mapping, replayable SSE, state projection, and app-scoped lifecycle reconciliation. A confirmed admission returns the same management handle whether projected as async `202` or sync setup-timeout `504`; an indeterminate journal launch returns that committed handle as async `202` instead of a raw failure. Timer scans are durable-cursor-bounded; request fast paths reuse targeted reconciliation. `reclaim_backing` covers active loss, idle teardown, and deletion of verified owned snapshot resources; a persisted sandbox omitted from the current inventory page is point-read before backing is considered absent. Due idle reclaim loads the session's run pages and fails closed on an incomplete bounded scan. Terminal success adoption and rearm preserve the shared result hold before reclaim. Terminal submit operations and intact terminal reclaim recovery remain fenced through remote idle-policy application and the final completion EGT, while page-local run absence never triggers ready-session reclaim. Reconciliation routes terminal successes through the persisted run agent slug's resolved output validator before adoption; invalid journal protocol terminalizes the matching run before quarantining the session and emits a redacted typed failure. | `RequestBudget`, `submit_run()`, `render_events()`, `SessionReconciler` |
| `azure_functions_agents/controller/readiness.py` | Validates an owner-scoped durable session against live controller state, the configured Sandbox Group, and the sandbox manifest before admission. `provision_new_session_submit()` reserves the first run before provider create, confirms ambiguous Table acknowledgements before any provider work, persists its sandbox target after discovery, and advances through lifecycle/content/manifest/journal phases under a stable label. Setup-budget expiry after reservation returns the durable handle instead of discarding it. Existing submissions use one `submit_run` fence through disarm, admission, journal acceptance, terminal rearm, and completion; terminal finalization requires the expected run ID before touching the active fence. A disarmed idle marker is valid only while its matching durable submit operation is active; an orphan marker fails closed. Attach/resume retain their transport handshake. | `SessionRuntimeBinding`, `activate_session()`, `provision_new_session_submit()`, `begin_submit_operation()`, `finalize_submit_operation()`, `revalidate_before_submit()` |
...
| `azure_functions_agents/transport/*` | Controller-to-sandbox boundary. Provider-neutral file/process/session ports include complete lifecycle policy, inventory, snapshot list/delete, and service metadata; the existing six file verbs remain unchanged. `aca_sdk.py` constructs regional clients directly from the authored Sandbox Group resource ID and region, with no ARM discovery. It is the only preview-SDK adapter and translates every provider failure into runtime-owned authorization, binding, transient, missing-backing, invalid-state, or provisioning errors before returning to controller/execution code. Provision requests may carry a stable operation label; the adapter discovers that label before retrying ambiguous creation, and any unavailable post-acceptance lookup remains outcome-unknown so reconciliation-only retries never issue another create while the label is temporarily invisible. | `SandboxSessionProvider`, `SandboxSessionHandle`, `SandboxLifecyclePolicy`, `SandboxSummary`, `SandboxSnapshot`, `AcaSandboxAdapter` |
...
| `azure_functions_agents/controller/{sandbox_config,bootstrap_delivery}.py` | Builds the immutable app-scoped `SandboxCreateProfile` (the Function interpreter's public `python-3.13`/`python-3.14` disk alias or an explicit customer disk, positive-provenance environment, one-shot bootstrap entrypoint, and create-time egress) and extends the provision-content fence with verified bootstrap delivery. The app archive and its full dependency closure cross the file plane; no custom bootstrap image is built. | `SandboxCreateProfile`, `build_sandbox_create_profile()`, `deliver_content_and_bootstrap()` |
...
| `azure_functions_agents/experimental/hybrid_{config,tools,protocol,executor,apim,observability,reaper}.py` | Private FRD 0009 spike: validates the opt-in and optional bundle-root boundaries, acquires one fresh ACA Sandbox per top-level run, delivers the deterministic package once, verifies its SHA-256 inside the stdlib executor before readiness, discovers customer tools only in the sandbox, routes exact runtime-owned MAF stubs through strict file journals, sends model traffic through APIM, and reaps crash orphans by stable app identity plus owner kind. Generic shell/file/search uses the same protocol; remote MCP remains worker-side. Normal cleanup arms terminal lifecycle and requests deletion without awaiting the LRO; confirmed fallback deletion never invalidates completed output. | `open_hybrid_invocation()`, `InvocationSandboxLease`, `HybridToolMiddleware`, `run_executor()`, `build_hybrid_chat_client()`, `reap_hybrid_orphans()` |
...
- `execution/` answers **"once registration hands off a request, which backend actually runs it?"** — `in_lang_worker` remains the default; the opt-in `aca_sandbox` backend implements the same exact four-method seam.
...
12. `registration/triggers.py` and `registration/endpoints.py` mutate one `FunctionApp` instance until all agents are registered, threading the frozen `AgentCatalog` through so handler closures can build `delegate_<slug>` tools later, at request time.

That ordering matters because later modules assume earlier stages have already reduced free-form author input into typed, validated objects. For example, registration code does not re-parse YAML or front matter; it trusts `ResolvedAgent` and `AgentCapabilities`. Steps 6-11 are FRD 0007's "two-pass composition" (§4.2) plus FRD 0008's startup gate: everything through step 11 is pure/side-effect-free (no `FunctionApp` mutation), and only step 12 is pass 2.
...
6. **Validate the `session_runtime` startup contract**
...
against a real Azurite Table service. The ACA binding uses these contracts for
owner resolution, durable state, and state projection without adding those values
to `StartRunRequest` or changing the exact four-method `AgentExecutionBackend`
abstraction. Missing `aca_sandbox` still selects `in_lang_worker` with unchanged
behavior; P3a/P3b introduce no ACA SDK import, and `azure.data.tables`
...
and failures are not cached. The ACA backend invokes this work lazily;
...
sandbox/generation after normal disk auto-suspend.
