...
| 89 | Status size under a maximal expansion | Emit failure detail for every instance / cap it deterministically | Cap it at the first 20 continued failures in persisted plan order, then instance index. `MAX_NODES` instances with maximal failure fields approach Durable's 16 KB custom-status limit, and an order-based cap keeps the status a pure function of persisted history | Agent, architecture review | 2026-08-29 |
| 90 | Untrusted policy fields on the early-failure span | Attach them as read / admit them only inside their validated domain | Admit only in-domain values. `max_attempts` and `continue_on_error` are metric dimensions, and the one path that reports them without validation is the `handler_contract` path, whose payload is by definition the one that just failed validation; an out-of-domain value there would create a bogus metric series rather than a bogus span attribute. Identifiers stay best-effort because they are span-only | Agent, code review | 2026-08-29 |
