...
| 111 | Which review conventions become lint rules vs. written conventions | Enable every candidate rule and mass-refactor the repo / rely on human review only / enable rules that pass repo-wide and document the rest | Enable `TRY002`, `TRY201`, `TRY203`, `TRY301`, and `TRY400`, which pass repo-wide (one scoped `per-file-ignores` entry for pre-existing `runner.py` findings this PR does not otherwise touch). Reject `TRY003` (~269 pre-existing sites; fights the established typed-exception-with-descriptive-message idiom), `TRY004` (7 sites where a domain exception is deliberately raised instead of `TypeError`), `ANN401` (460 pre-existing `Any` usages outside `transport/`), `PLR1702` (requires repo-wide `preview = true`, which alone surfaces 126 unrelated findings), and `C901` (15 pre-existing complex functions, 14 outside this PR's scope). Rejected rules become written rules in AGENTS.md instead, so enforcement never forces a repo-wide refactor this phase does not own. Rationale lives here rather than as commentary in `pyproject.toml`. | Agent + Human (reviewer requested the config stay terse) | 2026-07-30 | P4a (0008.5) |
