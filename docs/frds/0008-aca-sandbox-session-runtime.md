...
| 110 | Typed-boundary vs. untrusted-input parsing | Remove all runtime validation uniformly / keep it all / distinguish trusted-typed sources from untrusted input | Re-validating an already-typed source (SDK responses, our own dataclasses) is defensive coding and was deleted in favor of typing the boundary. Parsing genuinely untrusted input — the sandbox-written session manifest, the forgery-detection surface — stays strict. | Agent | 2026-07-30 | P4a (0008.5) |
