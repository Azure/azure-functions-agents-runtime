...
* Defaults: auto-suspend around 5 min; idle reclaim around 24 h. Both clocks start at terminal transition. `session.expires_at` is monotonic and is the authoritative upper bound for all result-hold deadlines: terminal success adoption preserves at least the shared 300-second hold, and touch plus submit/reclaim rearm may extend but never shorten it. Reclaim runs through suspend and any request—including polls—resets idle time. Thus active polling can retain a session indefinitely in v1; no absolute creation-time TTL.
...
* Reconciler: stale heartbeat verification, no false abandon, label-scoped platform divergence, exact off-page active/idle operation backing reads, per-sandbox lifecycle writes, no-live-delete CAS, backstop inequality, terminal/tombstone and completed-operation pruning, snapshot pruning, capacity reap-and-retry, resumed incomplete operations, required operation fields, fail-closed orphan idle markers, and reclaim convergence with more than 1,000 historical runs while unexpired result/idempotency retention remains intact. Fault injection covers each provider create/lifecycle/content/manifest/journal phase and stable-label recovery after ambiguous create or launch response loss.
