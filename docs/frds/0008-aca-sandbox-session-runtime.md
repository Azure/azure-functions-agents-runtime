...
| 191 | Qualification pipeline home | Extend `official-build.yml` / new pipeline / extend `e2e-tests.yml` | Move the five ACA stages to `e2e-tests.yml`, revising #182. Deploying a build to real Function Apps and qualifying it is end-to-end testing; official-build builds and releases artifacts. That pipeline already produces the `azurefunctions-agents-runtime` artifact the deploy consumes and already passes the ACA service connection, so the move needs no new build work. | Human | 2026-08-20 | #166 |
| 192 | ACA stage gating on a PR-triggered pipeline | Ungated / CI-only / CI-plus-manual | `e2e-tests.yml` has PR triggers, so gate every ACA stage: CI on `main` only, `Manual` allowed on any branch to keep the stages testable pre-merge, `PullRequest` and `Schedule` excluded. `release/*` deferred. Repeat the condition per stage rather than hoisting it into one variable, because a wrong expression would silently skip or silently run all five and a skipped stage reports the same green as one that never applied. | Human | 2026-08-20 | #166 |
| 193 | 1ES DefaultDeny network isolation | Ignore / CloudTest migration / allow-list request | The policy is an allow-list keyed by pipeline **definition ID** (`Policies/azfunc/policymap.json`), not by pool: 1733 and 1777 share pool and image but differ in enforcement. It blocks `management.<region>.azuredevcompute.io`, which lifecycle, loss, and load call from the agent to observe sandbox state. Moving to an unenrolled definition is a reprieve, not a fix; pursue endpoint allow-listing separately. CloudTest is the prescribed remedy for test-owned endpoints, which this is not. | Human | 2026-08-20 | #166 |
| 194 | Reconciler deadline argument on the management read path | Pass two arguments when absent / always pass the deadline | Always pass the deadline, widening the reconciler type to accept `None`. The two-argument branch raised `TypeError` against the registered three-parameter reconciler, returning 500 from status, result, and events; a `type: ignore[call-arg]` on that call had suppressed the mypy error describing it. Doubles mirror the real arity, since a `*args` double accepts any arity and cannot detect a mismatch. | Agent | 2026-08-21 | #166 |
