...
| 107 | `FileInfo.mode`'s pinned-SDK type annotation vs. its live wire behavior | Trust the SDK's `str \| None` annotation and coerce/reject at read time / model `mode` as `int \| None` and cast at the one read site / drop `mode` projection entirely | `azure-containerapps-sandbox==0.1.0b4` types `FileInfo.mode` as `str \| None`, but `FileInfo._from_dict` does a verbatim JSON passthrough (`mode=d.get("mode")`) with no coercion, so the real runtime type is whatever the wire sends. A human-authorized live run against real ACA observed `mode=420` (an int, POSIX `0o644`). The SDK's annotation is defective for this field; our runtime `SandboxFileEntry`/`SandboxFileStat.mode: int \| None` modeling is correct and is kept. `transport/aca_sdk.py`'s `_sdk_file_mode()` is the one place permitted to `cast()` past the incorrect stub, with a comment recording this decision. A prior review that "fixed" this back to `str` by hand-building a fake string payload was itself incorrect and must not be reintroduced. | Agent | 2026-07-30 | P4a (0008.5) |
| 108 | Canonical `SESSION_MANIFEST_PATH` wire contract | Leave the path unspecified per-implementation / pick a path scoped to this runtime's package name / reuse a generic `/var/lib/<generic>` path | Fix the in-sandbox manifest path at exactly `/var/lib/azurefunctions-agents-runtime/session/manifest.json`, exported as `transport.manifest.SESSION_MANIFEST_PATH`. This is a wire contract shared with the P4b/P5 harness (the process that writes the manifest from inside the sandbox): both sides must agree on the literal path with no coordination mechanism other than this constant, so the harness side must match this exact string. | Human | 2026-07-30 | P4a / P4b (0008.5 / 0008.6) |
| 109 | Module naming for modules sharing the bare name `models.py` | Keep both as bare `models.py`, disambiguated only by package / rename every offender in one PR / establish the convention now and rename only the modules this phase owns | Establish a globally-unique, intent-revealing module-naming convention: no two modules should share a bare name like `models.py` in this codebase. P4a adopts it now by renaming `transport/models.py` → `transport/transport_models.py` (via `git mv`, preserving history) and its mirroring test module to match. `session_state/models.py` → `session_models.py` is **deferred, not done here**: that module belongs to the separately-merged P3a phase and has active in-flight P3b work on another branch, so renaming it in this PR would create a merge collision. That rename is owned by the P3b phase, not P4a. | Human | 2026-07-30 | P4a (0008.5); `session_state` rename owned by P3b |
| 110 | Typed-boundary vs. untrusted-input parsing in the P4a transport review | Treat all runtime validation the same and remove it uniformly / keep all validation as-is / distinguish trusted-typed sources from untrusted external input | `getattr`/`isinstance`/`cast`-based re-validation of an **already-typed** source (preview SDK response objects, our own frozen dataclasses) is defensive coding and was deleted in favor of importing/typing the boundary and reading fields directly (Decision #107's `mode` cast is the sole documented exception). Strict parsing of **genuinely untrusted external input** — the sandbox session manifest read from inside a live sandbox in `transport/manifest.py`, the forgery/repointing detection surface — is parsing, not defensive coding, and stays fully intact: required-field checks, duplicate-JSON-key rejection, malformed group-ID rejection, and redacted mismatch errors are all still enforced with no tamper case newly accepted. | Agent | 2026-07-30 | P4a (0008.5) |
| 111 | Which review conventions become lint rules vs. written conventions | Enable every candidate rule and mass-refactor the repo / rely on human review only / enable rules that pass repo-wide and document the rest | Enable `TRY002`, `TRY201`, `TRY203`, `TRY301`, and `TRY400`, plus `D200`, `D210`, and `D419` for terse docstrings — all pass repo-wide (one scoped `per-file-ignores` entry for pre-existing `runner.py` findings this PR does not otherwise touch). Reject `TRY003` (~269 pre-existing sites; fights the established typed-exception-with-descriptive-message idiom), `TRY004` (7 sites where a domain exception is deliberately raised instead of `TypeError`), `ANN401` (460 pre-existing `Any` usages outside `transport/`), `PLR1702` (requires repo-wide `preview = true`, which alone surfaces 126 unrelated findings), `C901` (15 pre-existing complex functions, 14 outside this PR's scope), and the verbose-docstring rules `D205`/`D209` (93 and 27 pre-existing) plus `D400`/`D403`/`D415` (1-2 each, all pre-existing). Rejected rules become written rules in AGENTS.md instead, so enforcement never forces a repo-wide refactor this phase does not own. Rationale lives here rather than as commentary in `pyproject.toml`. | Agent + Human (reviewer requested the config stay terse) | 2026-07-30 | P4a (0008.5) |
...
* **P3a:** pure owner/app identity, canonical hashes, durable Table keys/row schemas, generation contract, snapshot encoding, and golden/negative vectors; no live storage or ACA calls.
* **P3b:** Azure Table I/O plus Azurite CAS race/409, admission EGT, idempotency, and tombstone/410 behavior.
* **P3c:** live-manifest, region, generation, and state-store-fingerprint/epoch binding.
