...
| 88 | Row 1 (`harness`) enforcement moves from `validate_session_runtime` to the schema layer | (a) leave `harness: str`, keep the runtime `!= "maf"` check in `validate_session_runtime`; (b) type `harness` as `Literal["maf"]` (matching the `EndpointAuthMode` convention) and remove the now-unreachable runtime check | (b) | Reviewer (PR #128) | 2026-07-29 | 0008.10 |
| 89 | P3a app/owner canonical bytes and stable Function App identity | delimiter concatenation / canonical JSON / fixed binary framing over normalized platform identity | v1 uses a 4-byte unsigned big-endian component count followed by, for each component, a 4-byte unsigned big-endian byte length and NFC UTF-8 bytes. `a1` frames the portable app tuple (see Decision 93); `o1` is discriminator-first and frames `("entra_user","o1",app_hash,agent_slug,tid,oid)` or `("function_app","o1",app_hash,agent_slug)`, so app/slot identity is defined once by `a1` and `o1` remains independently app-bound even outside a Table partition. GUIDs are canonical lower-case UUIDs; site/slot segments are NFC-normalized and lower-case. For `WEBSITE_SLOT_NAME`, unset/empty or a case-insensitive raw value of `production` produces the empty production-slot component; any other value is normalized as a slot name. Missing/invalid stable app inputs fail closed. An `entra_user` owner requires exactly one valid `tid` and immutable `oid`; missing/invalid/conflicting claims fail closed rather than framing an empty component. Digests are lower-case SHA-256 with `a1-` / `o1-` prefixes. Registries retain canonicalizers by stored version and verification never eagerly migrates a row. Function/admin-key auth selects `function_app` ownership (app-owned shared sessions); it never binds ownership to key material or key name. | Agent (P3a implementation) | 2026-07-30 | 0008.2 / P3a |
| 90 | P3a server IDs and durable Table key layout | opaque UUID text with ad-hoc joins / delimiter-safe bounded IDs with typed key parsers | Server-minted session/run IDs are 32-character lower-case UUID4 hex. Accepted IDs retain the existing `[A-Za-z0-9._-]{1,128}` contract, which excludes `:`, `/`, `\`, `#`, `?`, controls, and overlong values. `owner_hash_version` is the literal `o1`; the partition is exactly `o1:{app_hash}:{owner_kind}:{owner_hash}` (for example `o1:a1-<64hex>:function_app:o1-<64hex>`). Row keys are exactly `session:{session_id}`, `run:{session_id}:{run_id}`, and `idem:{session_id}:{lowercase_sha256(idempotency_key)}` (for example `session:<32hex>`). Idempotency keys are non-empty and at most 1024 UTF-8 bytes; only their digest is retained. Session, run, and optional idempotency rows for one admission must carry the identical owner partition so a later Azure Table entity-group transaction is legal. | Agent (P3a implementation) | 2026-07-30 | 0008.2 / 0008.3 / P3a |
| 91 | P3a durable row schema, generation transition, and snapshot-list encoding | unversioned loose dictionaries / immutable typed schema with bounded deterministic serialization | Table entities carry `schema_version=1` and UTC-aware timestamps. The session row carries the master fields plus `region`, credential-free `state_store_fingerprint` (`s1-<sha256>`), `quarantine_reason`, `tombstone_reason`, `created_at`, and `updated_at`; run rows carry admission generation, exact seven-state run status, result availability, bounded status reason, expiry, and timestamps; idempotency rows carry only the hashed key locator, request hash, run locator, expiry, and timestamps. Optional strings serialize as empty strings. Generation is an integer >=1; ordinary mutations must preserve it and a future state-preserving backing rebind must strictly increase it. `snapshot_ids` is row-schema-v1 compact JSON (`ensure_ascii=False`, separators `,`/`:`), ordered as supplied, limited to 64 IDs and 8192 UTF-8 bytes, and fails explicitly rather than truncating. P3a validates only the fingerprint's credential-free `s1-` shape; P3c defines the canonical state-store identity bytes it hashes. P3a defines and validates these pure serialization contracts only; P3b owns Table I/O/CAS/EGT, P3c owns live-manifest, region, and storage-epoch binding, and P3d owns reconciliation. | Agent (P3a implementation) | 2026-07-30 | 0008.3 / P3a |
| 92 | P3a architecture precision review | leave `o1` app input implicit / inline app fields again / bind `o1` to the defined `a1` hash and reconcile phase ownership | Bind `o1` to `app_hash`, define production-slot sentinel handling and Entra claim failure explicitly, add worked key shapes, defer state-store fingerprint bytes and live region validation to P3c, and split the old monolithic P3 gate into P3a-P3d. With these corrections the identity/key/schema contract is implementation-ready without widening P3a. | Agent reviewer | 2026-07-30 | P3a review |
| 93 | Portable Function App identity is subscription + site + slot only (revises #89 app inputs; no SKU conditionals) | (a) keep `subscription + resource_group + site + slot` via `WEBSITE_RESOURCE_GROUP`; (b) SKU-adaptive include RG only when present; (c) lowest common denominator with no conditionals: `subscription_id` from `WEBSITE_OWNER_NAME` prefix + `WEBSITE_SITE_NAME` + optional `WEBSITE_SLOT_NAME`; never hostname, never resource group, never function-key material | (c). Live probes across Flex Consumption, Elastic Premium, and Standard apps showed `WEBSITE_SITE_NAME`, `WEBSITE_SLOT_NAME`, and `WEBSITE_OWNER_NAME` present on all reachable runtimes, while `WEBSITE_RESOURCE_GROUP` is missing on every Flex app. Therefore `a1` frames exactly `("app","a1",subscription_id,site_name,slot_or_empty)`. Function/admin-key auth remains only the signal that ownership kind is `function_app` (the app owns the session; any valid key holder for that app shares those sessions by design). Ownership is never bound to a key value or key name, so key rotation does not orphan sessions. Easy Auth continues to select per-user `entra_user` ownership. No SKU branches. | Human + Agent | 2026-07-30 | 0008.2 / P3a |
| 94 | Easy Auth without a stable user subject fails closed (revises 0008.2 fallback language) | (a) keep historical 0008.2 fallback: Easy Auth principal without stable user subject becomes `function_app`; (b) fail closed with 401 when Entra mode cannot resolve exactly one stable `tid` + immutable `oid` | (b). Aligns session ownership with existing endpoint Entra enforcement: an `entra` route already rejects missing/invalid principals rather than silently widening to app-owned access. Function/admin-key requests still resolve `function_app` ownership; only authenticated app-key traffic is app-owned. A broken or subject-less Easy Auth principal must not inherit shared app sessions. | Human + Agent reviewer | 2026-07-30 | 0008.2 / P3a |
| 95 | App/agent rename has no automatic session migration in v1 | (a) automatic re-key/migration on site or agent-slug rename; (b) no migration — renamed app/agent is a new identity space | (b). `a1` includes site name and `o1` includes `agent_slug`, so renaming the Function App site or agent source slug changes hashes/partitions. Old durable rows remain under the previous identity until ordinary reclaim/pruning and are not automatically visible under the new name. Callers must start new sessions after rename. Explicit migration/alias tooling is deferred. | Human + Agent reviewer | 2026-07-30 | 0008.2 / P3a |
| 96 | Session-row lifecycle invariants for `active_run_id` | (a) shape-only validation; (b) enforce per-status active-run consistency on the durable session row | (b). `running` and `canceling` require a non-empty `active_run_id`. Idle/terminal statuses (`creating`, `ready`, `suspending`, `suspended`, `resuming`, `failed`, `quarantined`, `tombstoned`, `deleting`, `deleted`) require `active_run_id is None`. This keeps the schema from representing contradictory control state before P3b lifecycle I/O lands. | Agent reviewer | 2026-07-30 | 0008.3 / P3a |
| 97 | P4a direct file/process transport split | Put file operations behind `exec` / expose a mixed file-and-process port / retain exactly six direct file verbs and a separate process port | Keep `list_files`, `stat_file`, `read_file`, `write_file`, `delete_file`, and `mkdir` as the exact data-plane file Protocol; keep process `exec` separate so no journal I/O can shell out. This implements Decision #72 without changing the four-method execution seam. | Agent | 2026-07-30 | P4a (0008.5) |
| 98 | P4a live-manifest trust boundary | Derive owner/app/digest values in transport / trust a live handle alone / compare opaque authoritative controller values to a strict live manifest and handle identity | P4a consumes P3 owner/app/generation and P4b digest values as opaque inputs, strictly parses the live manifest, and raises redacted typed mismatches for every routing-critical field. It neither mutates Tables/CAS nor canonicalizes fingerprints or digests. | Agent | 2026-07-30 | P4a (0008.3 / 0008.6) |
| 99 | P4a group and resume verification sequence | Trust configured ARM ID / trust `get().state` / validate configured+persistent+ARM group identity/region then use direct manifest data-plane readiness | Bind only an ARM-resolved customer-owned group; create/attach/resume cross-check persisted group/region and live sandbox ID. Resume readiness is attach-by-persisted-ID → direct manifest `read_file` → strict binding verification; `get().state` remains advisory. | Agent | 2026-07-30 | P4a (0008.4 / 0008.8) |
| 100 | P4a safe create boundary | SDK defaults / implicit disk or ingress / explicit source, no ports, bounded setup, deny+inspected egress | Require one explicit `disk`/`disk_id`/`snapshot_id`/`preset`, explicit empty ports, `skip_egress_proxy=False`, `EgressPolicy(default_action="Deny", traffic_inspection="Full"|"Partial")`, and `min(remaining, 30s)` polling. Only the versioned owner hash and opaque app/session identifiers cross labels; the sandbox receives no Azure/state credential. **Narrowed by Decision #102** (below): `snapshot_id` is rejected in P4a, so the supported sources are `disk`/`disk_id`/`preset` only. | Agent | 2026-07-30 | P4a (0008.4 / 0008.9) |
| 101 | P4a preview verification gate | Simulate the preview SDK / require live ACA before non-live work / test double plus held credential-gated smoke | Keep the `0.1.0b4` SDK optional and confined to one production adapter, prove it through test-only injected factories and import guards, and hold the real create→file→exec→stop→resume→delete smoke until a human authorizes customer group and credential use. | Agent | 2026-07-30 | P4a (0008.5) |
| 102 | Snapshot restore at the P4a safe-create boundary (narrows #100 after review against pinned SDK source) | Forward `snapshot_id` despite incompatible options / omit required safety metadata / reject snapshot-backed session creation | Reject `snapshot_id` in P4a. SDK `0.1.0b4` snapshot restore forbids labels and egress policy because it replays captured state, so P4a cannot simultaneously prove the versioned owner binding and explicit deny-by-default inspected egress. Disk, disk ID, and preset remain supported. | Agent review | 2026-07-30 | P4a (0008.4 / 0008.9) |
| 103 | Readiness after the SDK's non-blocking `resume()` | Single immediate manifest read / SDK state poll / bounded direct data-plane retry followed by strict manifest verification | Keep the direct file response authoritative: issue `resume()`, retry only transient manifest-read failures within a caller-supplied finite deadline, then strictly verify the manifest and live handle. Do not use advisory state as the readiness decision. Stop/delete use explicit bounded LRO timeouts so tests and cleanup do not race non-blocking control calls. | Agent review | 2026-07-30 | P4a (0008.5 / 0008.8) |
| 104 | Reconcile a create that fails after the service accepted it | Lose the remote ID / inspect SDK-private poller state / public label-filtered cleanup | Add a random, non-identity provisioning-attempt label to each create. On polling, transport, cancellation, or handle-projection failure, list only that attempt and await bounded deletion of every match before propagating the failure; if cleanup cannot be confirmed, fail explicitly. | Agent review | 2026-07-30 | P4a (0008.4 / 0008.5) |
| 105 | P4a ownership of the shared harness manifest shape | Require P4a's fields to be the entire JSON object / accept ambiguous duplicate keys / require P4a fields while permitting later typed sections | Treat P4a's routing binding as a required strict subset of the shared manifest because harness capabilities are owned by later phases. Reject duplicate JSON keys, missing/invalid routing fields, and malformed group IDs; ignore additional well-formed sections rather than claiming ownership of their schema. | Agent review | 2026-07-30 | P4a (0008.3 / 0008.6 / 0008.8) |
| 106 | Canonical owner/app fingerprint text after live ACA label validation (revises the `o<version>-<hex>` wording in #55) | Keep 67-character hex tokens and add a P4a-only projection / split tokens across labels / use one full-entropy base32 token everywhere | Encode the existing SHA-256 digest bytes as lowercase unpadded base32, yielding `o1-<52 chars>` and `a1-<52 chars>` (55 characters). Tables, manifests, paths, and ACA labels use the identical canonical value; P3b updates the canonicalizers and golden vectors, while P4a treats the values as opaque and enforces ACA's 63-character label limit before a service call. This preserves all 256 digest bits and version semantics without a second transport identity. | Human | 2026-07-30 | P3b / P4a (0008.2 / 0008.4) |
| 107 | `FileInfo.mode` contradicts its pinned-SDK annotation | Trust the `str \| None` annotation / model `int \| None` and cast at the one read site / drop the field | `0.1.0b4` annotates `mode` as `str \| None`, but `_from_dict` passes the wire value through uncoerced and a live run observed `420` (int). Model it `int \| None`; `_sdk_file_mode()` is the only permitted cast past the bad stub. | Agent | 2026-07-30 | P4a (0008.5) |
| 108 | Canonical in-sandbox manifest path | Leave it per-implementation / scope it to this runtime's package name / reuse a generic path | Fix it at `/var/lib/azurefunctions-agents-runtime/session/manifest.json`, exported as `SESSION_MANIFEST_PATH`. Wire contract: the harness writes this path from inside the sandbox and has no other coordination mechanism. | Human | 2026-07-30 | P4a / P4b (0008.5 / 0008.6) |
| 109 | Modules must not share a bare name like `models.py` | Keep both, disambiguated by package / rename every offender at once / set the convention and rename only what this phase owns | Adopt globally-unique, intent-revealing module names. Rename `transport/models.py` → `transport_models.py` here. `session_state/models.py` is deferred to the phase that owns it, to avoid colliding with in-flight work. | Human | 2026-07-30 | P4a (0008.5); `session_state` rename deferred |
| 110 | Typed-boundary vs. untrusted-input parsing | Remove all runtime validation uniformly / keep hand-rolled validators / declare a strict typed model | Re-validating an already-typed source (SDK responses, our own dataclasses) is defensive coding and was deleted in favor of typing the boundary. Untrusted input — the sandbox-written manifest — is parsed by a strict Pydantic model instead of hand-rolled checks: same fail-closed guarantee, declared once so it cannot drift. | Agent + Human | 2026-07-30 | P4a (0008.5) |
| 112 | Parsing the untrusted sandbox manifest | Hand-rolled `isinstance`/`cast` validators / `model_validate_json` / `json.loads` with a duplicate-key hook, then a strict model | Decode with `json.loads(object_pairs_hook=...)` to reject duplicate keys — both `json.loads` and Pydantic's JSON parser silently keep the last value, so one document could otherwise mean two things — then validate with a `strict=True`, `extra="ignore"` model. Strict blocks type coercion; `extra="ignore"` leaves later manifest sections to the harness. `ValidationError` is never surfaced: it embeds the offending values, so failures still collapse to one redacted error. | Human | 2026-07-31 | P4a (0008.5) |
| 111 | Which conventions become lint rules vs. written rules | Enable every candidate and refactor the repo / rely on review only / enable what passes repo-wide, document the rest | Enable `TRY002/201/203/301/400` and `D200/D210/D419` (all zero-violation). Reject `TRY003`, `TRY004`, `ANN401`, `PLR1702`, `C901`, `D205/209/400/403/415` — each has pre-existing violations that would force a repo-wide refactor this phase does not own, so they become AGENTS.md rules instead. | Agent + Human | 2026-07-30 | P4a (0008.5) |
| 113 | P3b's validator and serialization precision for Decision #106's canonical base32 encoding | (a) accept lenient base32 variants (mixed case, padded) defensively at read time; (b) strict canonical-only encoding: lower-case, unpadded, fixed-length, reject everything else | (b). Decision #106 (already recorded jointly by P3b/P4a) approved replacing the 67-character hex `a1-`/`o1-` framing with one canonical base32 payload used everywhere because ACA rejects labels over 63 characters; this decision records the concrete implementation contract P3b builds against. Base32-encode (RFC 4648, lower-cased, `=`-stripped) the SAME full 256-bit SHA-256 digest bytes Decision #89 already defines -- `ceil(256/5) = 52` characters, so every `a1-`/`o1-` token is 55 characters total, preserving full digest entropy (unlike truncating hex, which would throw entropy away). The `a1`/`o1` version discriminators, framing (`frame_canonical_components`), and canonicalizer-registry/no-eager-migration behavior from Decision #89 are UNCHANGED -- only the digest's string encoding changes, from hex to base32. Strict validators accept ONLY canonical lower-case `[a-z2-7]{52}` payloads and reject uppercase, `=` padding, and malformed lengths. Because no released/live P3 state exists yet on `feature/aca-sandboxes`, no hex-to-base32 migration path is built; all P3a golden vectors, validators, and worked examples are updated in place. | Agent (P3b implementation) | 2026-07-30 | 0008.2 / 0008.3 / P3b (implements #106) |
| 114 | Non-secret state-store fingerprint (`s1-`) uses the same label-safe base32 encoding, computed by P3b (moves fingerprint computation earlier than Decision #91's P3c placeholder) | (a) leave `state_store_fingerprint` bytes/encoding undefined until P3c; (b) define and compute it now in P3b, versioned `s1-<52 lower-case base32>`, from normalized non-secret Table account/endpoint identity | (b). P3b's Table-connection-resolution module needs a concrete, real fingerprint value to populate and test session rows end-to-end (Azurite round trips, golden vectors) rather than leaving the field an untestable placeholder. `compute_state_store_fingerprint` hashes `frame_canonical_components(("s1", normalized_endpoint, normalized_account_name))` -- the same delimiter-safe framing Decision #89 uses for `a1`/`o1` -- then base32-encodes the digest exactly like Decisions #106/#113. The normalized inputs come only from the resolved `TableServiceClient`'s `.url`/`.account_name` (never the connection string or credential/key material); userinfo-embedded URLs fail closed, and query/fragment components are always stripped before hashing. Same account with a rotated key/credential yields the identical fingerprint; a different account or endpoint always differs. This narrows, but does not complete, Decision #91's P3c scope: P3b defines and computes the fingerprint's bytes/encoding; P3c still owns binding it into live-manifest/region/storage-epoch verification (i.e., cross-checking the stored fingerprint against the sandbox's live view), which remains unimplemented until P3c. | Agent (P3b implementation) | 2026-07-30 | 0008.3 / P3b |
| 115 | `AzureWebJobsStorage` Table connection precedence and client caching (P3b) | (a) require only the connection-string form; (b) support both connection-string and identity-based forms with an explicit precedence and non-secret client cache key | (b). Mirrors `_blob_history.py`'s existing precedence exactly (table-flavored): a non-empty `AzureWebJobsStorage` connection string wins outright (covers Azurite/dev storage and real accounts alike); otherwise `AzureWebJobsStorage__tableServiceUri` selects identity-based access, with managed-identity client selection precedence `AzureWebJobsStorage__clientId` -> `AZURE_CLIENT_ID` -> bare `DefaultAzureCredential()`. Neither configured is a hard `StateStoreConfigurationError` (fail closed; no in-memory/local fallback, consistent with Decision #86 having no dedicated-account concept). `TableServiceClient`/`TableClient` instances are cached process-wide keyed ONLY by the computed non-secret `s1-` fingerprint (Decision #114) -- never by connection string, URI, or credential -- so two different secrets for the same underlying account safely share one cached client, and a connection string is never held as or logged via a cache key. | Agent (P3b implementation) | 2026-07-30 | 0008.3 / P3b |
| 116 | P3b Table I/O, ETag/CAS, admission EGT, and terminal-adoption contract | (a) best-effort sequential writes with app-level locking; (b) Azure Table entity-group transactions (EGT) with ETag/CAS for every state-changing operation, no best-effort sequences | (b). `admit_run` deduplicates by idempotency key BEFORE checking `active_run_id` (same key + same request-payload hash replays the existing run; same key + different payload is a typed `IdempotencyConflictError`; a distinct/absent key while a run is already active is a typed `ActiveRunConflictError` carrying the current `run_id` after a safe re-read), then admits via one EGT that conditionally updates the session (`active_run_id: null -> run_id`), creates the run row, and creates the idempotency row (when present) -- all sharing one owner partition per Decision #90. `adopt_terminal_run` is idempotent (safe to call redundantly from inline resubmit, opportunistic cleanup, and the future reconciler timer alike): it is a no-op when the run is already terminal with the SAME outcome, a typed `TerminalStateConflictError` when already terminal with a DIFFERENT outcome, and otherwise an EGT that marks the run terminal and clears the session's `active_run_id` only if the session still shows that run as active (session status always moves to `ready` when the slot is released, per Decision #96's active-run-status invariant). `update_session`/`tombstone_session` validate the generation transition (preserve, or strictly increase only for a future state-preserving backing rebind) before any write, mapping a rollback attempt to a typed `GenerationConflictError`. Every Table SDK failure is mapped to a narrow typed error (not-found, already-exists, ETag/precondition conflict, corrupt entity, or a generic unavailable/auth/throttle/5xx catch-all) -- never a silently-swallowed or coerced success. Verified against real Azurite (not a fake) for every CAS/EGT/race claim above, including concurrent two-controller admission and terminal-adoption races. Live sandbox-manifest verification, region/storage-epoch binding, and reaper/reconciliation policy remain out of scope (P3c/P3d). | Agent (P3b implementation) | 2026-07-30 | 0008.2 / 0008.3 / P3b |
| 117 | Rename `session_state/models.py` to `session_state/session_models.py` (fulfills Decision #109's deferred session_state rename) | (a) leave as `models.py`, defer the rename indefinitely; (b) mechanically rename now, in P3b, since P3b is the next phase to touch this module (P4a explicitly declined to rename it as a drive-by in its own PR) | (b). Decision #109 (already recorded, P4a) renamed `transport/models.py` to `transport/transport_models.py`, adopted the repo-wide globally-unique intent-revealing module-name convention, and explicitly deferred `session_state/models.py`'s rename to whichever phase touches that module next. A human approved doing it here via one mechanical `git mv` (history-preserving): every importer (`session_state/__init__.py`, `identity.py`, `connection.py`, `store.py`, and `registration/_auth.py`'s dormant owner-principal seam) and the mirroring test module (`tests/test_session_state_models.py` -> `tests/test_session_state_session_models.py`, per §6's `tests/test_<module>.py` convention) were updated in the same change. Rename-only: no behavior, public type, field, or schema change. Re-exports through `azure_functions_agents.session_state`'s `__init__.py` are unaffected -- external code importing the package (not the submodule path) sees no difference. | Human (relayed) + Agent (P3b implementation) | 2026-07-30 | P3b |
| 118 | Retire the stale `P3c`/`P3d` phase-slice labels from this FRD's living prose; fold their described scope into the already-existing reduced phase mapping | (a) leave `P3c`/`P3d` placeholder labels in living narrative/architecture prose indefinitely; (b) replace every living (non-historical) `P3c`/`P3d` mention with the reduced mapping already used elsewhere in this FRD and in `docs/architecture.md` | (b). `P3c` and `P3d` were provisional placeholder labels from Decision #92's original P3a-P3d split, coined before the phase plan converged on `P4a`/`P4b`/`P5a`/`P5b`/`P6`. They duplicate scope already owned by existing phases: live sandbox-manifest, region, generation, and state-store-fingerprint/epoch cross-verification (the former `P3c` scope) is `P4b`/`P5a`; reaper reconciliation over Table/platform truth and lifecycle repair (the former `P3d` scope) is `P6`. This is a pure wording/documentation correction -- no scope, ownership, or behavior change; no phase's content moves, `P3c`/`P3d` are simply retired as standalone slice names, and every already-decided row (`#91`, `#92`, `#114`, `#116`) that used `P3c`/`P3d` terminology at the time it was written is left untouched as an append-only historical record of that terminology's origin. Only living narrative sections (this FRD's "P3b implementation" summary and its "Phase ownership, gates, and tests" list, plus `docs/architecture.md`'s module map/prose) are updated to stop using the retired labels. | Agent (P3b PR #132-convention compliance pass) | 2026-07-30 | P3b |
...
- **P3b implementation (2026-07-30):** Following live P4a ACA smoke testing,
  ...
  are P4b/P5a, and reaper reconciliation is P6 (Decision 118 retires the
  earlier `P3c`/`P3d` placeholder slice labels).
  ...
  behavior or public-type change. A subsequent proactive-compliance pass
  (2026-07-30) verified this module already satisfies P4a's not-yet-merged
  ruff `TRY*` rules and its five new `AGENTS.md` conventions (no defensive
  typing against already-typed sources, `create()`-factory dataclasses, no
  nested try/except, unique module names, module constants over re-inlined
  literals), tightening a handful of `store.py` internal type annotations
  (`Any` to concrete SDK/domain types) with zero behavior change, and made
  this documentation correction (Decision 118).
...
* **P4b:** controller capture/zip/digest/delivery; deterministic digest, large package exemption, partial-delivery digest gate/retry; also owns live sandbox-manifest capture and verification against the Table-stored digest (folded in from the retired `P3c` placeholder, Decision #118).
* **P5a/P5b:** ACA backend then sync/async/SSE/budgets/idempotency; seven states, cursor error, 504 reason, all idempotency cases, disconnect safety, snapshot-restart, async failure 200; P5a also owns live region/generation/state-store-fingerprint/epoch cross-verification against the Table row before serving a request (folded in from the retired `P3c` placeholder, Decision #118).
