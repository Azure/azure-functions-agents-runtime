...
| 89 | P3a app/owner canonical bytes and stable Function App identity | delimiter concatenation / canonical JSON / fixed binary framing over normalized platform identity | v1 uses a 4-byte unsigned big-endian component count followed by, for each component, a 4-byte unsigned big-endian byte length and NFC UTF-8 bytes. `a1` frames `("app","a1",subscription_id,resource_group,site,slot_or_empty)`; `o1` is discriminator-first and frames `("entra_user","o1",app_hash,agent_slug,tid,oid)` or `("function_app","o1",app_hash,agent_slug)`, so app/slot identity is defined once by `a1` and `o1` remains independently app-bound even outside a Table partition. GUIDs are canonical lower-case UUIDs; ARM identity segments are NFC-normalized and lower-case. For `WEBSITE_SLOT_NAME`, unset/empty or a case-insensitive raw value of `production` produces the empty production-slot component; any other value is normalized as a slot name. The controller resolves the app from `WEBSITE_OWNER_NAME`'s subscription prefix, `WEBSITE_RESOURCE_GROUP`, `WEBSITE_SITE_NAME`, and optional `WEBSITE_SLOT_NAME`, never from hostname; missing/invalid stable app inputs fail closed. An `entra_user` owner requires exactly one valid `tid` and immutable `oid`; missing/invalid/conflicting claims fail closed rather than framing an empty component. Digests are lower-case SHA-256 with `a1-` / `o1-` prefixes. Registries retain canonicalizers by stored version and verification never eagerly migrates a row. | Agent (P3a implementation) | 2026-07-30 | 0008.2 / P3a |
