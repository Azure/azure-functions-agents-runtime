...
- [ ] Regression: repeated primary/delegated builds for one provider/model reuse
      ...
      (Issue #157, Decision #21). Concurrent real-wrapper runs cover
      streaming + streaming, non-streaming + non-streaming, and mixed calls,
      proving request/session/function-call/event isolation without serialization
      (Decision #24).
