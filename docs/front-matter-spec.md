...
- `session_runtime` — Object selecting the session execution backend for every agent in the app (in-process by default; configuring the nested `aca_sandbox` block selects the ACA Sandbox backend). Never configured per-agent. See [`session_runtime`](#session_runtime).
...
session_runtime:
  ...
  aca_sandbox:                       # Presence of this block selects the ACA Sandbox backend
    ...
    region: string                   # Required region selecting the group's data-plane endpoint
...
- **Description:** Points at a **pre-provisioned, customer-owned** Azure Container Apps Sandbox Group. Both `sandbox_group_resource_id` and `region` are required. The authored region identifies and selects the Sandbox Group's regional ACA data-plane endpoint directly. The Function App and Sandbox Group may be in the same or different regions. The runtime does not discover the region through ARM or provide a compatibility fallback.
...
letters and digits. It identifies the Sandbox Group's regional data-plane
endpoint. See the
...
- **Type:** `object`
- **Location:** Nested inside `aca_sandbox` — it cannot be authored anywhere else, so it is only ever present when the ACA Sandbox backend is already selected.
...
- Because `retention` is nested inside `aca_sandbox` rather than a sibling of it, "retention configured for the in-process backend" is not a case that can be authored at all — the schema itself rules it out.
...
Dynamic Workflows are not supported when the global
`session_runtime.aca_sandbox` block is configured. Startup rejects
`workflows.enabled: true` before any trigger or endpoint is registered;
declaring `workflows.subagents` does not bypass that gate. Omit `aca_sandbox`
to use Dynamic Workflows on the default in-language-worker path.
