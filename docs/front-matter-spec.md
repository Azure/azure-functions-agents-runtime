...
builtin_endpoints:
  ...
  a2a:                # Optional; explicit object required
    mode: simple      # Non-streaming, direct-Message profile
...
never implicit; it requires the object form with `url`. `mode` is optional and
defaults to `simple`.
...
The simple profile accepts only a non-streaming user Message containing text Parts and returns
...
Fixed safety bounds are 256 KiB per raw request, 16 Parts, 32 KiB per text
