...
- **Run agents on durable workflows** *(experimental, see [Dynamic workflows](workflows.md))* — one frontmatter flag turns on a DAG-of-tools execution model with deterministic retry, bounded timeouts, continued failure, and stable idempotency that fans out and survives restarts **without** burning tokens on intermediate results
