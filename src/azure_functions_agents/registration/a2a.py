"""Experimental native A2A 1.0 registration for the simple profile."""
...
async def _read_bounded_body(req: Request) -> bytes | None:
    """Read the request body, or return ``None`` when it is larger than the limit."""
...
def _replay_request(req: Request, body: bytes) -> Request:
    """Return a request that gives the already-read body to the SDK route."""
    sent = False
    ...
    return Request(req.scope, receive)


async def _preflight_request(req: Request) -> tuple[Response | None, bytes]:
    body = await _read_bounded_body(req)
    if body is None:
        return (
            _json_http_error(
    ...
    return _validate_request_body(req, body), body


def _validate_request_body(req: Request, body: bytes) -> Response | None:
    try:
...
def register_a2a_endpoints(
    ...
    *,
    ...
    """Register one per-agent card and JSON-RPC route for the simple profile."""
    ...
    async def handle_rpc(req: Request, durable_client: Any | None) -> Response:
        ...
        preflight_error, body = await _preflight_request(req)
        if preflight_error is not None:
            return preflight_error

        req.scope[_AUTH_SCOPE_STATE] = auth_scope
        req.scope[_DURABLE_CLIENT_STATE] = durable_client
        response = await sdk_rpc_endpoint(_replay_request(req, body))
