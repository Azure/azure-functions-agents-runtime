...
def test_sample_agent_authors_an_ordinary_workflow() -> None:
    agent_text = (_SAMPLE_SRC / "main.agent.md").read_text(encoding="utf-8")

    assert "start_workflow" in agent_text
    # Retry is declared on the tool, so nothing about it belongs in the prompt.
    assert "retry" not in agent_text.lower()


def test_sample_tools_are_discoverable_with_their_retry_declaration() -> None:
    clear_tool_discovery_cache()
...
def test_sample_plan_freezes_the_tool_declared_retry() -> None:
    plan = validate_plan(
        {
            "tasks": [
                {
    ...
    clear_tool_discovery_cache()
    discovered = discover_project_tools(_SAMPLE_SRC)
    declared = {tool.name: tool.retry for tool in discovered.workflow_tools}
    policy = WorkflowToolExecutionPolicy(retry=declared["reserve_inventory"])

    effective = resolve_workflow_task_execution(
        plan.tasks[0], decorator_retry=policy.retry
    )
