...
def _streaming_request(
    ...
    path = "/agents/incident-triage/a2a"
    request = Request(
        {
            "type": "http",
            "asgi": {"version": "3.0"},
...
@pytest.mark.asyncio
async def test_preflight_rejects_large_content_length_without_reading_body(
    registered_a2a: tuple[_FakeFunctionApp, list[dict[str, Any]]],
) -> None:
    app, calls = registered_a2a
    handler = _route(app, "/a2a")["handler"]
    request, reads = _streaming_request(
        [b"{}"], headers={"content-length": str(_MAX_REQUEST_BYTES + 1)}
    )
...
@pytest.mark.asyncio
async def test_preflight_stops_reading_chunked_body_at_limit(
    registered_a2a: tuple[_FakeFunctionApp, list[dict[str, Any]]],
) -> None:
    app, calls = registered_a2a
    handler = _route(app, "/a2a")["handler"]
    chunk = b" " * (_MAX_REQUEST_BYTES // 2 + 1)
    request, reads = _streaming_request([chunk, chunk, chunk, chunk])
