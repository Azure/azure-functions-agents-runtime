...
Adds one fixture-only ``/__buildinfo`` route to the standard agent app so a
qualification run can confirm that the app it is about to qualify is running
the build that was just deployed.
...
def _load_marker() -> dict[str, Any]:
    """Read the stamped marker, or report its absence explicitly."""
    try:
...
def _content_size() -> dict[str, Any]:
    ...
    Reported so a qualification run can trend closure growth toward the 256 MiB
