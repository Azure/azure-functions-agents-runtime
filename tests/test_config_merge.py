...
def test_app_wide_auth_is_inherited_by_agents() -> None:
    """A top-level agents.config.yaml `auth` becomes each agent's default."""
    global_config = GlobalConfig(auth=EndpointAuthConfig(mode="entra"))
    resolved = _resolve_builtin_endpoints(
        AgentSpec(name="A", description="B", builtin_endpoints=BuiltinEndpointsConfig(chat_api=True)),
        global_config,
    )
    assert resolved.auth.mode == "entra"
...
def test_app_wide_auth_inherited_for_shorthand_builtin_endpoints() -> None:
    ...
    global_config = GlobalConfig(auth=EndpointAuthConfig(mode="anonymous"))
    resolved = _resolve_builtin_endpoints(
        AgentSpec(name="A", description="B", builtin_endpoints=True), global_config
    )
    assert resolved.auth.mode == "anonymous"


def test_app_wide_auth_shorthand_string_is_coerced() -> None:
    """A bare-string `auth: entra` at the global level is coerced and inherited."""
    global_config = GlobalConfig.model_validate({"auth": "admin"})
    resolved = _resolve_builtin_endpoints(
        AgentSpec(name="A", description="B", builtin_endpoints=True), global_config
    )
    assert resolved.auth.mode == "admin"
    ...
    spec = AgentSpec.model_validate(
        {
            "name": "A",
            "description": "B",
            "builtin_endpoints": {"chat_api": True, "auth": "function"},
        }
    )
    resolved = _resolve_builtin_endpoints(spec, global_config)
    assert resolved.auth.mode == "function"
