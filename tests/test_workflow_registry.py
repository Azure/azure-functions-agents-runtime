...
@pytest.mark.parametrize(
    ...
    [
        (
            lambda workflow_id: tools.start_workflow.handler(
                ToolInvocation(
                    session_id="session-1",
                    arguments={
                        "tasks": [
        ...
        (
            lambda workflow_id: tools.get_workflow_status.handler(
                ToolInvocation(
                    session_id="session-1",
        ...
        (
            lambda workflow_id: tools.list_workflows.handler(
                ToolInvocation(session_id="session-1", arguments={})
            ),
            "failed to list workflows",
            "list_workflows: fetch_session_workflows failed",
        ),
        (
            lambda workflow_id: tools.terminate_workflow.handler(
                ToolInvocation(
                    session_id="session-1",
                    arguments={"workflow_id": workflow_id},
                )
        ...
        (
            lambda workflow_id: tools.cancel_workflow.handler(
                ToolInvocation(
                    session_id="session-1",
                    arguments={"workflow_id": workflow_id},
                )
...
async def test_workflow_tools_log_durable_exceptions_without_returning_details(
    ...
    result = await call_tool(workflow_id)
    text_result = result.text_result_for_llm
...
async def test_start_workflow_rejects_new_workflow_when_session_active_cap_reached():
    ...
    token = context.register_workflow_session(session_id, "test-agent", client)
    registry.set_app_config(frozenset())
    try:
        result = await tools.start_workflow.handler(
            ToolInvocation(
