...
def test_build_async_credential_passes_managed_identity_client_id_when_env_set(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setenv("AZURE_CLIENT_ID", "client-id-123")

    credential = object()
    with mock.patch("azure.identity.aio.DefaultAzureCredential", return_value=credential) as factory:
        from azure_functions_agents._credential import build_async_credential

        assert build_async_credential() is credential

    factory.assert_called_once_with(managed_identity_client_id="client-id-123")


def test_build_async_credential_returns_bare_default_credential_when_env_unset(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.delenv("AZURE_CLIENT_ID", raising=False)
